...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/les/flowcontrol"
...
type LesServer struct {
	...
	fcCoslbchain-devats     *requestCoslbchain-devats
...
func NewLesServer(lbchain-dev *lbchain-dev.lbchain-devchain, config *lbchain-dev.Config) (*LesServer, error) {
	quitSync := make(chan struct{})
	pm, err := NewProtocolManager(lbchain-dev.BlockChain().Config(), false, ServerProtocolVersions, config.NetworkId, lbchain-dev.EventMux(), lbchain-dev.Engine(), newPeerSet(), lbchain-dev.BlockChain(), lbchain-dev.TxPool(), lbchain-dev.ChainDb(), nil, nil, quitSync, new(sync.WaitGroup))
	if err != nil {
		return nil, err
	}
	...
	for i, pv := range AdvertiseProtocolVersions {
		lesTopics[i] = lesTopic(lbchain-dev.BlockChain().Genesis().Hash(), pv)
	}
	...
	srv := &LesServer{
		...
		quitSync:         quitSync,
		lesTopics:        lesTopics,
		chtIndexer:       light.NewChtIndexer(lbchain-dev.ChainDb(), false),
		bloomTrieIndexer: light.NewBloomTrieIndexer(lbchain-dev.ChainDb(), false),
	}
	logger := log.New()
	...
	srv.chtIndexer.Start(lbchain-dev.BlockChain())
	pm.server = srv
	...
	srv.fcCoslbchain-devats = newCoslbchain-devats(lbchain-dev.ChainDb())
	return srv, nil
}
...
type requestCoslbchain-devats struct {
	lock  sync.RWMutex
	db    lbchain-devdb.Database
	stats map[uint64]*linReg
}

type requestCoslbchain-devatsRlp []struct {
	MsgCode uint64
	Data    []byte
}

var rcStatsKey = []byte("_requestCoslbchain-devats")

func newCoslbchain-devats(db lbchain-devdb.Database) *requestCoslbchain-devats {
	...
	if db != nil {
		...
		var statsRlp requestCoslbchain-devatsRlp
		if err == nil {
	...
	return &requestCoslbchain-devats{
		db:    db,
		stats: stats,
	}
}

func (s *requestCoslbchain-devats) store() {
	s.lock.Lock()
	defer s.lock.Unlock()

	statsRlp := make(requestCoslbchain-devatsRlp, len(reqList))
...
func (s *requestCoslbchain-devats) getCurrentList() RequestCostList {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *requestCoslbchain-devats) update(msgCode, reqCnt, cost uint64) {
	s.lock.Lock()
	defer s.lock.Unlock()
