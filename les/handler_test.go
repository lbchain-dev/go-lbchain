...
func testGetCHTProofs(t *testing.T, protocol int) {
	...
	case 1:
		...
		trie, _ := trie.New(root, trie.NewDatabase(lbchain-devdb.NewTable(db, light.ChtTablePrefix)))
...
func TestGetBloombitsProofs(t *testing.T) {
	...
	for bit := 0; bit < 2048; bit++ {
		...
		trie, _ := trie.New(root, trie.NewDatabase(lbchain-devdb.NewTable(db, light.BloomTrieTablePrefix)))
