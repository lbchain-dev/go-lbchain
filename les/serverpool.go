...
const (
	...
	// inilbchain-devatsWeight is used to initialize previously unknown peers with good
	// statistics to give a chance to prove themselves
	inilbchain-devatsWeight = 1
)
...
type serverPool struct {
	...
	adjuslbchain-devats          chan poolStatAdjust
...
func newServerPool(db lbchain-devdb.Database, quit chan struct{}, wg *sync.WaitGroup) *serverPool {
	pool := &serverPool{
		db:           db,
		quit:         quit,
		wg:           wg,
		...
		adjuslbchain-devats:  make(chan poolStatAdjust, 100),
...
func (pool *serverPool) disconnect(entry *poolEntry) {
	...
	if entry.state == psRegistered {
		...
		if stopped {
			entry.conneclbchain-devats.add(1, connAdjust)
		} else {
			entry.conneclbchain-devats.add(connAdjust, 1)
		}
	}
...
func (pool *serverPool) adjustBlockDelay(entry *poolEntry, time time.Duration) {
	...
	pool.adjuslbchain-devats <- poolStatAdjust{pseBlockDelay, entry, time}
}
...
func (pool *serverPool) adjustResponseTime(entry *poolEntry, time time.Duration, timeout bool) {
	...
	if timeout {
		pool.adjuslbchain-devats <- poolStatAdjust{pseResponseTimeout, entry, time}
	} else {
		pool.adjuslbchain-devats <- poolStatAdjust{pseResponseTime, entry, time}
	}
}
...
func (pool *serverPool) eventLoop() {
	...
	for {
		...
		case adj := <-pool.adjuslbchain-devats:
			pool.lock.Lock()
			...
			case pseResponseTime:
				...
				adj.entry.timeoulbchain-devats.add(0, 1)
			case pseResponseTimeout:
				adj.entry.timeoulbchain-devats.add(1, 1)
			}
			pool.lock.Unlock()
...
func (pool *serverPool) findOrNewNode(id discover.NodeID, ip net.IP, port uint16) *poolEntry {
	...
	if entry == nil {
		...
		entry.conneclbchain-devats.add(1, inilbchain-devatsWeight)
		entry.delayStats.add(0, inilbchain-devatsWeight)
		entry.responseStats.add(0, inilbchain-devatsWeight)
		entry.timeoulbchain-devats.add(0, inilbchain-devatsWeight)
	}
...
func (pool *serverPool) loadNodes() {
	...
	for _, e := range list {
		log.Debug("Loaded server stats", "id", e.id, "fails", e.lastConnected.fails,
			"conn", fmt.Sprintf("%v/%v", e.conneclbchain-devats.avg, e.conneclbchain-devats.weight),
			"delay", fmt.Sprintf("%v/%v", time.Duration(e.delayStats.avg), e.delayStats.weight),
			"response", fmt.Sprintf("%v/%v", time.Duration(e.responseStats.avg), e.responseStats.weight),
			"timeout", fmt.Sprintf("%v/%v", e.timeoulbchain-devats.avg, e.timeoulbchain-devats.weight))
		pool.entries[e.id] = e
...
func (pool *serverPool) checkDialTimeout(entry *poolEntry) {
	...
	entry.conneclbchain-devats.add(0, 1)
	entry.dialed.fails++
	pool.setRetryDial(entry)
}
...
type poolEntry struct {
	...
	conneclbchain-devats, delayStats    poolStats
	responseStats, timeoulbchain-devats poolStats
	state                       int
...
func (e *poolEntry) EncodeRLP(w io.Writer) error {
	return rlp.Encode(w, []interface{}{e.id, e.lastConnected.ip, e.lastConnected.port, e.lastConnected.fails, &e.conneclbchain-devats, &e.delayStats, &e.responseStats, &e.timeoulbchain-devats})
}
...
func (e *poolEntry) DecodeRLP(s *rlp.Stream) error {
	var entry struct {
		...
		Port                       uint16
		Fails                      uint
		CStat, DStat, RStat, lbchain-devat poolStats
	}
	...
	e.conneclbchain-devats = entry.CStat
	e.delayStats = entry.DStat
	e.responseStats = entry.RStat
	e.timeoulbchain-devats = entry.lbchain-devat
...
func (e *knownEntry) Weight() int64 {
	...
	return int64(1000000000 * e.conneclbchain-devats.recentAvg() * math.Exp(-float64(e.lastConnected.fails)*failDropLn-e.responseStats.recentAvg()/float64(responseScoreTC)-e.delayStats.recentAvg()/float64(delayScoreTC)) * math.Pow(1-e.timeoulbchain-devats.recentAvg(), timeoutPow))
}
