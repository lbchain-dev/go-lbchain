...
func newTestProtocolManager(lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db lbchain-devdb.Database) (*ProtocolManager, error) {
	var (
	...
	if lightSync {
		...
		bloomIndexer := lbchain-dev.NewBloomIndexer(db, params.BloomBitsBlocks)
	...
	if !lightSync {
		...
		srv.fcCoslbchain-devats = newCoslbchain-devats(nil)
	}
	pm.Start(1000)
	return pm, nil
}
...
// fails the test.
func newTestProtocolManagerMust(t *testing.T, lightSync bool, blocks int, generator func(int, *core.BlockGen), peers *peerSet, odr *LesOdr, db lbchain-devdb.Database) *ProtocolManager {
