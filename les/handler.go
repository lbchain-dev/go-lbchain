...
const (
	...
	lbchain-devVersion = 63 // equivalent lbchain-dev version for the downloader
...
func NewProtocolManager(chainConfig *params.ChainConfig, lightSync bool, protocolVersions []uint, networkId uint64, mux *event.TypeMux, engine consensus.Engine, peers *peerSet, blockchain BlockChain, txpool txPool, chainDb lbchain-devdb.Database, odr *LesOdr, txrelay *LesTxRelay, quitSync chan struct{}, wg *sync.WaitGroup) (*ProtocolManager, error) {
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light lbchain-devchain protocol")
	...
	log.Info("Light lbchain-devchain protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	p.Log().Debug("Light lbchain-devchain peer connected", "name", p.Name())
	...
	if err := p.Handshake(td, hash, number, genesis.Hash(), pm.server); err != nil {
		p.Log().Debug("Light lbchain-devchain handshake failed", "err", err)
		return err
	}
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light lbchain-devchain peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light lbchain-devchain message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light lbchain-devchain message arrived", "code", msg.Code, "bytes", msg.Size)
	...
	case GetBlockHeadersMsg:
		...
		pm.server.fcCoslbchain-devats.update(msg.Code, query.Amount, rcost)
	...
	case GetBlockBodiesMsg:
		...
		pm.server.fcCoslbchain-devats.update(msg.Code, uint64(reqCnt), rcost)
	...
	case GetHeaderProofsMsg:
		...
		trieDb := trie.NewDatabase(lbchain-devdb.NewTable(pm.chainDb, light.ChtTablePrefix))
		for _, req := range req.Reqs {
		...
		for _, req := range req.Reqs {
			if auxTrie == nil || req.Type != lastType || req.TrieIdx != lastIdx {
				...
				if root, prefix = pm.getHelperTrie(req.Type, req.TrieIdx); root != (common.Hash{}) {
					auxTrie, _ = trie.New(root, trie.NewDatabase(lbchain-devdb.NewTable(pm.chainDb, prefix)))
				}
			}
...
func (d *downloaderPeerNotify) registerPeer(p *peer) {
	...
	pm.downloader.RegisterLightPeer(p.id, lbchain-devVersion, pc)
}
