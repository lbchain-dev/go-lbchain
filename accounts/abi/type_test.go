...
// typeWithoulbchain-devringer is a alias for the Type type which simply doesn't implement
// the stringer interface to allow printing type details in the tests below.
type typeWithoulbchain-devringer Type
...
func TestTypeRegexp(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if !reflect.DeepEqual(typ, tt.kind) {
			t.Errorf("type %q: parsed type mismatch:\nGOT %s\nWANT %s ", tt.blob, spew.Sdump(typeWithoulbchain-devringer(typ)), spew.Sdump(typeWithoulbchain-devringer(tt.kind)))
		}
	}
}
