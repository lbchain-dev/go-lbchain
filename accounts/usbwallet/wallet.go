...
type driver interface {
	...
	// Derive sends a derivation request to the USB device and returns the lbchain-devchain
...
type wallet struct {
	...
	deriveChain    lbchain-devereum.ChainStateReader // Blockchain state reader to discover used account with
...
// whlbchain-dever they are still present or if they malfunctioned.
...
func (w *wallet) selfDerive() {
	...
	for errc == nil && err == nil {
		...
		for empty := false; !empty; {
			// Retrieve the next derived lbchain-devchain account
			...
			// If the next account is empty, stop self-derivation, but add it nonlbchain-develess
...
// Contains implements accounts.Wallet, returning whlbchain-dever a particular account is
...
func (w *wallet) SelfDerive(base accounts.DerivationPath, chain lbchain-devereum.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()
