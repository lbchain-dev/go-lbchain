...
// https://raw.githubusercontent.com/LedgerHQ/blue-app-lbchain-dev/master/doc/lbchain-devapp.asc
...
const (
	ledgerOpRetrieveAddress  ledgerOpcode = 0x02 // Returns the public key and lbchain-devchain address for a given BIP 32 path
	ledgerOpSignTransaction  ledgerOpcode = 0x04 // Signs an lbchain-devchain transaction after having the user validate the parameters
...
type ledgerDriver struct {
	...
	browser bool          // Flag whlbchain-dever the Ledger is in browser mode (reply channel mismatch)
...
func (w *ledgerDriver) Status() (string, error) {
	...
	if w.browser {
		return "lbchain-devchain app in browser mode", w.failure
	}
	if w.offline() {
		return "lbchain-devchain app offline", w.failure
	}
	return fmt.Sprintf("lbchain-devchain app v%d.%d.%d online", w.version[0], w.version[1], w.version[2]), w.failure
}

// offline returns whlbchain-dever the wallet and the lbchain-devchain app is offline or not.
//
...
func (w *ledgerDriver) Open(device io.ReadWriter, passphrase string) error {
	...
	if err != nil {
		// lbchain-devchain app is not running or in browser mode, nothing more to do, return
		if err == errLedgerReplyInvalidHeader {
			w.browser = true
		}
		return nil
	}
	// Try to resolve the lbchain-devchain app's version, will fail prior to v1.0.2
...
// and returning the lbchain-devchain address located on that derivation path.
...
//
// Note, if the version of the lbchain-devchain application running on the Ledger wallet is
// too old to sign EIP-155 transactions, but such is requested nonlbchain-develess, an error
// will be returned opposed to silently signing in Homestead mode.
func (w *ledgerDriver) SignTx(path accounts.DerivationPath, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	// If the lbchain-devchain app doesn't run, abort
	if w.offline() {
...
// ledgerVersion retrieves the current version of the lbchain-devchain wallet app running
...
// ledgerDerive retrieves the currently active lbchain-devchain address from a Ledger
...
//   lbchain-devchain address length | 1 byte
//   lbchain-devchain address        | 40 bytes hex ascii
...
func (w *ledgerDriver) ledgerDerive(derivationPath []uint32) (common.Address, error) {
	...
	// Extract the lbchain-devchain hex address string
	if len(reply) < 1 || len(reply) < 1+int(reply[0]) {
		return common.Address{}, errors.New("reply lacks address entry")
	}
	hexstr := reply[1 : 1+int(reply[0])]

	// Decode the hex sting into an lbchain-devchain address and return
...
func (w *ledgerDriver) ledgerSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	// Create the transaction RLP based on whlbchain-dever legacy or EIP155 signing was requeste
	var (
		txrlp []byte
		err   error
	)
	...
	// Extract the lbchain-devchain signature and do a sanity validation
	if len(reply) != 65 {
