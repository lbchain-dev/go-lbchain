...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/accounts/usbwallet/internal/trezor"
...
type trezorDriver struct {
	...
	pinwait bool          // Flags whlbchain-dever the device is waiting for PIN entry
...
// Status implements accounts.Wallet, always whlbchain-dever the Trezor is opened, closed
// or whlbchain-dever the lbchain-devchain app was not started on it.
...
// lbchain-devchain address located on that path.
func (w *trezorDriver) trezorDerive(derivationPath []uint32) (common.Address, error) {
	address := new(trezor.lbchain-devchainAddress)
	if _, err := w.trezorExchange(&trezor.lbchain-devchainGetAddress{AddressN: derivationPath}, address); err != nil {
...
func (w *trezorDriver) trezorSign(derivationPath []uint32, tx *types.Transaction, chainID *big.Int) (common.Address, *types.Transaction, error) {
	...
	request := &trezor.lbchain-devchainSignTx{
	...
	response := new(trezor.lbchain-devchainTxRequest)
	...
	for response.DataLength != nil && int(*response.DataLength) <= len(data) {
		...
		if _, err := w.trezorExchange(&trezor.lbchain-devchainTxAck{DataChunk: chunk}, response); err != nil {
			return common.Address{}, nil, err
		}
	}
