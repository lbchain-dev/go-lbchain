...
var (
	initCommand = cli.Command{
		...
		Name:      "init",
		Usage:     "Boolbchain-devrap and initialize a new genesis block",
...
Use "lbchain-devereum dump 0" to dump the genesis block.`,
	}
)
...
func importChain(ctx *cli.Context) error {
	...
	db := chainDb.(*lbchain-devdb.LDBDatabase)
...
func copyDb(ctx *cli.Context) error {
	...
	db, err := lbchain-devdb.NewLDBDatabase(ctx.Args().First(), ctx.GlobalInt(utils.CacheFlag.Name), 256)
	if err != nil {
		return err
	}
	...
	start = time.Now()
	fmt.Println("Compacting entire database...")
	if err = chainDb.(*lbchain-devdb.LDBDatabase).LDB().CompactRange(util.Range{}); err != nil {
		utils.Fatalf("Compaction failed: %v", err)
	}
	fmt.Printf("Compaction done in %v.\n\n", time.Since(start))
