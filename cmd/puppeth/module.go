...
// returned from docker inspect, parsed into a form easily usable by pupplbchain-dev.
type containerInfos struct {
	running bool              // Flag whlbchain-dever the container is running currently
...
func inspectContainer(client *sshClient, container string) (*containerInfos, error) {
	// Check whlbchain-dever there's a container running for the service
	...
	// Infos retrieved, parse the above into somlbchain-deving meaningful
