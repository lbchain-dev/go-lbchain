...
FROM pupplbchain-dev/wallet:latest

ADD genesis.json /genesis.json

RUN \
  echo 'node server.js &'                     > wallet.sh && \
	echo 'glbchain-dev --cache 512 init /genesis.json' >> wallet.sh && \
	echo $'glbchain-dev --networkid {{.NetworkID}} --port {{.NodePort}} --bootnodes {{.Bootnodes}} --lbchain-devstats \'{{.lbchain-devstats}}\' --cache=512 --rpc --rpcaddr=0.0.0.0 --rpccorsdomain "*"' >> wallet.sh

RUN \
	sed -i 's/Pupplbchain-devNetworkID/{{.NetworkID}}/g' dist/js/lbchain-deverwallet-master.js && \
	sed -i 's/Pupplbchain-devNetwork/{{.Network}}/g'     dist/js/lbchain-deverwallet-master.js && \
	sed -i 's/Pupplbchain-devDenom/{{.Denom}}/g'         dist/js/lbchain-deverwallet-master.js && \
	sed -i 's/Pupplbchain-devHost/{{.Host}}/g'           dist/js/lbchain-deverwallet-master.js && \
	sed -i 's/Pupplbchain-devRPCPort/{{.RPCPort}}/g'     dist/js/lbchain-deverwallet-master.js
...
// checkWallet does a health-check against web wallet server to verify whlbchain-dever
// it's running, and if yes, whlbchain-dever it's responsive.
