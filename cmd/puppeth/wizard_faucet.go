...
func (w *wizard) deployFaucet() {
	...
	fmt.Println()
	fmt.Printf("How many lbchain-devers to release per request? (default = %d)\n", infos.amount)
	...
	if infos.node.datadir == "" {
		...
		infos.node.datadir = w.readDefaullbchain-devring(infos.node.datadir)
	}
	// Figure out which port to listen on
	fmt.Println()
	...
	fmt.Println()
	if infos.node.lbchain-devstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.node.lbchain-devstats = w.readString() + ":" + w.conf.lbchain-devstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.node.lbchain-devstats)
		infos.node.lbchain-devstats = w.readDefaullbchain-devring(infos.node.lbchain-devstats) + ":" + w.conf.lbchain-devstats
	}
	...
	infos.noauth = w.readDefaullbchain-devring(noauth) != "n"
