...
func (w *wizard) deployDashboard() {
	...
	for _, service := range []string{"lbchain-devstats", "explorer", "wallet", "faucet"} {
		...
		for _, server := range available[service] {
			...
			var port int
			switch service {
			case "lbchain-devstats":
				if infos, err := checklbchain-devstats(client, w.network); err == nil {
					port = infos.port
				}
			case "explorer":
	...
	// If we have lbchain-devstats running, ask whlbchain-dever to make the secret public or not
	if w.conf.lbchain-devstats != "" {
		fmt.Println()
		fmt.Println("Include lbchain-devstats secret on dashboard (y/n)? (default = yes)")
		infos.trusted = w.readDefaullbchain-devring("y") == "y"
	}
