...
func (w *wizard) gatherStats(server string, pubkey []byte, client *sshClient) *serverStat {
	...
	logger.Debug("Checking for lbchain-devstats availability")
	if infos, err := checklbchain-devstats(client, w.network); err != nil {
		if err != ErrServiceUnknown {
			stat.services["lbchain-devstats"] = map[string]string{"offline": err.Error()}
		}
	} else {
		stat.services["lbchain-devstats"] = infos.Report()
		lbchain-devstats = infos.config
	}
	...
	if lbchain-devstats != "" {
		w.conf.lbchain-devstats = lbchain-devstats
	}
