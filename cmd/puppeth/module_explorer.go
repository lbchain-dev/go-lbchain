...
FROM pupplbchain-dev/explorer:latest

ADD lbchain-devstats.json /lbchain-devstats.json
ADD chain.json /chain.json

RUN \
  echo '(cd ../lbchain-dev-net-intelligence-api && pm2 start /lbchain-devstats.json)' >  explorer.sh && \
	echo '(cd ../lbchain-deverchain-light && npm start &)'                      >> explorer.sh && \
	echo '/parity/parity --chain=/chain.json --port={{.NodePort}} --tracing=on --fat-db=on --pruning=archive' >> explorer.sh

ENTRYPOINT ["/bin/sh", "explorer.sh"]
`

// explorerlbchain-devstats is the configuration file for the lbchain-devstats javascript client.
var explorerlbchain-devstats = `[
  {
    "name"              : "node-app",
    "script"            : "app.js",
...
services:
  explorer:
    ...
    ports:
      ...
      - "{{.WebPort}}:3000"{{end}}
    volumes:
      - {{.Datadir}}:/root/.local/share/io.parity.lbchain-devereum
    environment:
      - NODE_PORT={{.NodePort}}/tcp
      - STATS={{.lbchain-devstats}}{{if .VHost}}
      - VIRTUAL_HOST={{.VHost}}
...
func deployExplorer(client *sshClient, network string, chainspec []byte, config *explorerInfos, nocache bool) ([]byte, error) {
	...
	lbchain-devstats := new(bytes.Buffer)
	template.Must(template.New("").Parse(explorerlbchain-devstats)).Execute(lbchain-devstats, map[string]interface{}{
		"Port":   config.nodePort,
		"Name":   config.lbchain-devstats[:strings.Index(config.lbchain-devstats, ":")],
		"Secret": config.lbchain-devstats[strings.Index(config.lbchain-devstats, ":")+1 : strings.Index(config.lbchain-devstats, "@")],
		"Host":   config.lbchain-devstats[strings.Index(config.lbchain-devstats, "@")+1:],
	})
	files[filepath.Join(workdir, "lbchain-devstats.json")] = lbchain-devstats.Bytes()
...
func checkExplorer(client *sshClient, network string) (*explorerInfos, error) {
	...
	stats := &explorerInfos{
		datadir:  infos.volumes["/root/.local/share/io.parity.lbchain-devereum"],
		nodePort: nodePort,
		webHost:  host,
		webPort:  webPort,
		lbchain-devstats: infos.envvars["STATS"],
	}
	return stats, nil
}
