...
func (w *wizard) deployNode(boot bool) {
	...
	if w.conf.lbchain-devstats == "" {
		log.Error("No lbchain-devstats server configured")
		return
	}
	...
	if infos.datadir == "" {
		...
		infos.datadir = w.readDefaullbchain-devring(infos.datadir)
	}
	if w.conf.Genesis.Config.lbchain-devash != nil && !boot {
		fmt.Println()
		if infos.ethashdir == "" {
			...
			infos.ethashdir = w.readDefaullbchain-devring(infos.ethashdir)
		}
	}
	...
	fmt.Println()
	if infos.lbchain-devstats == "" {
		fmt.Printf("What should the node be called on the stats page?\n")
		infos.lbchain-devstats = w.readString() + ":" + w.conf.lbchain-devstats
	} else {
		fmt.Printf("What should the node be called on the stats page? (default = %s)\n", infos.lbchain-devstats)
		infos.lbchain-devstats = w.readDefaullbchain-devring(infos.lbchain-devstats) + ":" + w.conf.lbchain-devstats
	}
	// If the node is a miner/signer, load up needed credentials
	if !boot {
		if w.conf.Genesis.Config.lbchain-devash != nil {
			// lbchain-devash based miners only need an lbchain-deverbase to mine against
			fmt.Println()
			if infos.lbchain-deverbase == "" {
				fmt.Printf("What address should the miner user?\n")
				for {
					if address := w.readAddress(); address != nil {
						infos.lbchain-deverbase = address.Hex()
						break
					}
				}
			} else {
				fmt.Printf("What address should the miner user? (default = %s)\n", infos.lbchain-deverbase)
				infos.lbchain-deverbase = w.readDefaultAddress(common.HexToAddress(infos.lbchain-deverbase)).Hex()
			}
	...
	if out, err := deployNode(client, w.network, w.conf.bootnodes, infos, nocache); err != nil {
		log.Error("Failed to deploy lbchain-devchain node container", "err", err)
		if len(out) > 0 {
			fmt.Printf("%s\n", out)
		}
		return
