...
// config contains all the configurations needed by pupplbchain-dev that should be saved
// between sessions.
type config struct {
	path      string   // File containing the configuration values
	bootnodes []string // Bootnodes to always connect to by all nodes
	lbchain-devstats  string   // lbchain-devstats settings to cache for node deploys
...
func (c config) flush() {
	...
	if err := ioutil.WriteFile(c.path, out, 0644); err != nil {
		log.Warn("Failed to save pupplbchain-dev configs", "file", c.path, "err", err)
	}
}
...
type wizard struct {
	...
	services map[string][]string   // lbchain-devchain services known to be running on servers
...
// readDefaullbchain-devring reads a single line from stdin, trimming if from spaces. If
// an empty line is entered, the default value is returned.
func (w *wizard) readDefaullbchain-devring(def string) string {
	fmt.Printf("> ")
	text, err := w.in.ReadString('\n')
	if err != nil {
		log.Crit("Failed to read user input", "err", err)
...
func (w *wizard) readDefaultBigInt(def *big.Int) *big.Int {
	for {
		...
		val, ok := new(big.Int).Selbchain-devring(text, 0)
		if !ok {
...
// it to an lbchain-devchain address.
func (w *wizard) readAddress() *common.Address {
	for {
		...
		bigaddr, _ := new(big.Int).Selbchain-devring(text, 16)
...
// converts it to an lbchain-devchain address. If an empty line is entered, the default
// value is returned.
...
// weird formats used by lbchain-devstats, which compares IPs textually, not by value.
