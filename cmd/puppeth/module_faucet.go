...
FROM lbchain-devereum/client-go:alltools-latest
...
ENTRYPOINT [ \
	"faucet", "--genesis", "/genesis.json", "--network", "{{.NetworkID}}", "--bootnodes", "{{.Bootnodes}}", "--lbchain-devstats", "{{.lbchain-devstats}}", "--lbchain-devport", "{{.lbchain-devPort}}",     \
...
services:
  faucet:
    ...
    ports:
      - "{{.lbchain-devPort}}:{{.lbchain-devPort}}"{{if not .VHost}}
      - "{{.ApiPort}}:8080"{{end}}
    volumes:
      - {{.Datadir}}:/root/.faucet
    environment:
      - lbchain-dev_PORT={{.lbchain-devPort}}
      - lbchain-dev_NAME={{.lbchain-devName}}
...
func deployFaucet(client *sshClient, network string, bootnodes []string, config *faucetInfos, nocache bool) ([]byte, error) {
	...
	template.Must(template.New("").Parse(faucetDockerfile)).Execute(dockerfile, map[string]interface{}{
		...
		"lbchain-devstats":      config.node.lbchain-devstats,
		"lbchain-devPort":       config.node.port,
	...
	template.Must(template.New("").Parse(faucetComposefile)).Execute(composefile, map[string]interface{}{
		...
		"ApiPort":       config.port,
		"lbchain-devPort":       config.node.port,
		"lbchain-devName":       config.node.lbchain-devstats[:strings.Index(config.node.lbchain-devstats, ":")],
		"CaptchaToken":  config.captchaToken,
		"CaptchaSecret": config.captchaSecret,
		"FaucetAmount":  config.amount,
		"FaucetMinutes": config.minutes,
...
func (info *faucetInfos) Report() map[string]string {
	report := map[string]string{
		...
		"lbchain-devchain listener port":       strconv.Itoa(info.node.port),
		"Funding amount (base tier)":   fmt.Sprintf("%d lbchain-devers", info.amount),
		"Funding cooldown (base tier)": fmt.Sprintf("%d mins", info.minutes),
		"Funding tiers":                strconv.Itoa(info.tiers),
		"Captha protection":            fmt.Sprintf("%v", info.captchaToken != ""),
		"lbchain-devstats username":            info.node.lbchain-devstats,
	}
	if info.noauth {
...
// checkFaucet does a health-check against an faucet server to verify whlbchain-dever
...
func checkFaucet(client *sshClient, network string) (*faucetInfos, error) {
	...
	return &faucetInfos{
		node: &nodeInfos{
			...
			port:     infos.portmap[infos.envvars["lbchain-dev_PORT"]+"/tcp"],
			lbchain-devstats: infos.envvars["lbchain-dev_NAME"],
			keyJSON:  keyJSON,
			keyPass:  keyPass,
		},
		host:          host,
