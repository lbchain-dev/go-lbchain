...
// makeWizard creates and returns a new pupplbchain-dev wizard.
...
// setting up a new or managing an existing lbchain-devchain private network.
func (w *wizard) run() {
	fmt.Println("+-----------------------------------------------------------+")
	fmt.Println("| Welcome to pupplbchain-dev, your lbchain-devchain private network manager |")
	fmt.Println("|                                                           |")
	fmt.Println("| This tool lets you create a new lbchain-devchain network down to  |")
	fmt.Println("| the genesis block, bootnodes, miners and lbchain-devstats servers |")
	fmt.Println("| without the hassle that it would normally entail.         |")
	fmt.Println("|                                                           |")
	fmt.Println("| Pupplbchain-dev uses SSH to dial in to remote servers, and builds |")
	...
	log.Info("Administering lbchain-devchain network", "name", w.network)

	// Load initial configurations and connect to all live servers
	w.conf.path = filepath.Join(os.Getenv("HOME"), ".pupplbchain-dev", w.network)
	...
	for {
		...
		default:
			log.Error("That's not somlbchain-deving I can do")
		}
	}
}
