...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/cmd/utils"
	"github.com/lbchain-devchain/go-lbchain-dev/common"
	"github.com/lbchain-devchain/go-lbchain-dev/console"
	"github.com/lbchain-devchain/go-lbchain-dev/crypto"
	"github.com/lbchain-devchain/go-lbchain-dev/lbchain-devclient"
	"github.com/lbchain-devchain/go-lbchain-dev/internal/debug"
	...
	"github.com/lbchain-devchain/go-lbchain-dev/swarm"
	bzzapi "github.com/lbchain-devchain/go-lbchain-dev/swarm/api"
	swarmmetrics "github.com/lbchain-devchain/go-lbchain-dev/swarm/metrics"
...
var (
	...
	SwarmSwapAPIFlag = cli.StringFlag{
		Name:   "swap-api",
		Usage:  "URL of the lbchain-devchain API provider to use to settle SWAP payments",
	...
	Deprecatedlbchain-devAPIFlag = cli.StringFlag{
		Name:  "ethapi",
...
// This init function sets defaults so cmd/swarm can run alongside glbchain-dev.
func init() {
...
var app = utils.NewApp(gitCommit, "lbchain-devchain Swarm")
...
Export a local chunk database as a tar archive (use - to send to stdout).
    ...
    swarm db export ~/.lbchain-devereum/swarm/bzz-KEY/chunks chunks.tar

The export may be quite large, consider piping the output through the Unix
pv(1) tool to get a progress bar:

    swarm db export ~/.lbchain-devereum/swarm/bzz-KEY/chunks - | pv > chunks.tar
`,
				},
				{
					Action:    dbImport,
...
Import chunks from a tar archive into a local chunk database (use - to read from stdin).
    ...
    swarm db import ~/.lbchain-devereum/swarm/bzz-KEY/chunks chunks.tar

The import may be quite large, consider piping the input through the Unix
pv(1) tool to get a progress bar:

    pv chunks.tar | swarm db import ~/.lbchain-devereum/swarm/bzz-KEY/chunks -
`,
				},
				{
					Action:    dbClean,
...
func bzzd(ctx *cli.Context) error {
	...
	//glbchain-dev only supports --datadir via command line
	//in order to be consistent within swarm, if we pass --datadir via environment variable
	//or via config file, we get the same directory for glbchain-dev and swarm
	if _, err := os.Stat(bzzconfig.Path); err == nil {
		cfg.DataDir = bzzconfig.Path
	}
	//setup the lbchain-devereum node
	...
	//register BZZ as node.Service in the lbchain-devereum node
...
func registerBzzService(bzzconfig *bzzapi.Config, ctx *cli.Context, stack *node.Node) {
	...
	boot := func(ctx *node.ServiceContext) (node.Service, error) {
		var swapClient *lbchain-devclient.Client
		var err error
		if bzzconfig.SwapApi != "" {
			log.Info("connecting to SWAP API", "url", bzzconfig.SwapApi)
			swapClient, err = lbchain-devclient.Dial(bzzconfig.SwapApi)
			if err != nil {
	...
	//register within the lbchain-devereum node
...
func getAccount(bzzaccount string, ctx *cli.Context, stack *node.Node) *ecdsa.PrivateKey {
	...
	return decryplbchain-devoreAccount(ks, bzzaccount, utils.MakePasswordList(ctx))
}

func decryplbchain-devoreAccount(ks *keystore.KeyStore, account string, passwords []string) *ecdsa.PrivateKey {
