...
const (
	...
	Glbchain-dev_ENV_DATADIR          = "Glbchain-dev_DATADIR"
)
...
var tomlSettings = toml.Config{
	...
	MissingField: func(rt reflect.Type, field string) error {
		...
		if unicode.IsUpper(rune(rt.Name()[0])) && rt.PkgPath() != "main" {
			link = fmt.Sprintf(", check github.com/lbchain-devchain/go-lbchain-dev/swarm/api/config.go for available fields")
		}
...
func initSwarmNode(config *bzzapi.Config, stack *node.Node, ctx *cli.Context) {
	...
	//set the resolved config path (glbchain-dev --datadir)
...
func configFileOverride(config *bzzapi.Config, ctx *cli.Context) (*bzzapi.Config, error) {
	...
	//only do somlbchain-deving if the -config flag has been set
...
func envVarsOverride(currentConfig *bzzapi.Config) (config *bzzapi.Config) {
	...
	if datadir := os.Getenv(Glbchain-dev_ENV_DATADIR); datadir != "" {
		currentConfig.Path = datadir
	}
...
func checkDeprecated(ctx *cli.Context) {
	...
	if ctx.GlobalString(Deprecatedlbchain-devAPIFlag.Name) != "" {
...
func printConfig(config *bzzapi.Config) string {
	...
	if err != nil {
		return fmt.Sprintf("Somlbchain-deving is not right with the configuration: %v", err)
	}
	return string(out)
}
