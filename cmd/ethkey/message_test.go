...
func TestMessageSignVerify(t *testing.T) {
	tmpdir, err := ioutil.TempDir("", "lbchain-devkey-test")
	if err != nil {
	...
	// Create the key.
	generate := runlbchain-devkey(t, "generate", keyfile)
	generate.Expect(`
...
`)
	...
	// Sign a message.
	sign := runlbchain-devkey(t, "signmessage", keyfile, message)
	sign.Expect(`
!! Unsupported terminal, password will be echoed.
Passphrase: {{.InputLine "foobar"}}
`)
	...
	// Verify the message.
	verify := runlbchain-devkey(t, "verifymessage", address, signature, message)
