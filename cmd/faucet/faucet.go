...
// faucet is a lbchain-dever faucet backed by a light client.
package main
...
var (
	...
	lbchain-devPortFlag = flag.Int("lbchain-devport", 30303, "Listener port for the devp2p connection")
	bootFlag    = flag.String("bootnodes", "", "Comma separated bootnode enode URLs to seed with")
	netFlag     = flag.Uint64("network", 0, "Network ID to use for the lbchain-devchain protocol")
	statsFlag   = flag.String("lbchain-devstats", "", "lbchain-devstats network monitoring auth string")

	netnameFlag = flag.String("faucet.name", "", "Network name to assign to the faucet")
	payoutFlag  = flag.Int("faucet.amount", 1, "Number of lbchain-devers to pay out per user request")
	...
	logFlag    = flag.Int("loglevel", 3, "Log level to use for lbchain-devchain and the faucet")
)

var (
	lbchain-dever = new(big.Int).Exp(big.NewInt(10), big.NewInt(18), nil)
)
...
func main() {
	...
	for i := 0; i < *tiersFlag; i++ {
		...
		amounts[i] = fmt.Sprintf("%s lbchain-devers", strconv.FormatFloat(amount, 'f', -1, 64))
		if amount == 1 {
	...
	faucet, err := newFaucet(genesis, *lbchain-devPortFlag, enodes, *netFlag, *statsFlag, ks, website.Bytes())
	if err != nil {
...
type request struct {
	...
	Account common.Address     `json:"account"` // lbchain-devchain address being funded
	Time    time.Time          `json:"time"`    // Timestamp when the request was accepted
	Tx      *types.Transaction `json:"tx"`      // Transaction funding the account
}

// faucet represents a crypto faucet backed by an lbchain-devchain light client.
type faucet struct {
	config *params.ChainConfig // Chain configurations for signing
	stack  *node.Node          // lbchain-devchain protocol stack
	client *lbchain-devclient.Client   // Client connection to the lbchain-devchain chain
...
func newFaucet(genesis *core.Genesis, port int, enodes []*discv5.Node, network uint64, stats string, ks *keystore.KeyStore, index []byte) (*faucet, error) {
	...
	stack, err := node.New(&node.Config{
		...
		P2P: p2p.Config{
			...
			MaxPeers:         25,
			Boolbchain-devrapNodesV5: enodes,
		},
	})
	if err != nil {
		return nil, err
	}
	// Assemble the lbchain-devchain light client protocol
	if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
		cfg := lbchain-dev.DefaultConfig
	...
	// Assemble the lbchain-devstats monitoring and reporting service'
	if stats != "" {
		if err := stack.Register(func(ctx *node.ServiceContext) (node.Service, error) {
			var serv *les.Lightlbchain-devchain
			ctx.Service(&serv)
			return lbchain-devstats.New(stats, nil, serv)
		}); err != nil {
			return nil, err
		}
	}
	...
	client := lbchain-devclient.NewClient(api)
...
// close terminates the lbchain-devchain connection and tears down the faucet.
...
// apiHandler handles requests for lbchain-dever grants and transaction statuses.
func (f *faucet) apiHandler(conn *websocket.Conn) {
	...
	if err = send(conn, map[string]interface{}{
		"funds":    balance.Div(balance, lbchain-dever),
		"funded":   nonce,
	...
	for {
		...
		// Retrieve the lbchain-devchain address to fund, the requesting user and a profile picture
		var (
			username string
			avatar   string
		...
		default:
			err = errors.New("Somlbchain-deving funky happened, please open an issue at https://github.com/lbchain-devchain/go-lbchain-dev/issues")
		}
		if err != nil {
			if err = sendError(conn, err); err != nil {
		...
		if timeout = f.timeouts[username]; time.Now().After(timeout) {
			...
			amount := new(big.Int).Mul(big.NewInt(int64(*payoutFlag)), lbchain-dever)
...
func (f *faucet) loop() {
	...
	go func() {
		for head := range update {
			...
			balance = new(big.Int).Div(balance, lbchain-dever)
...
// the username, avatar URL and lbchain-devchain address to fund on success.
func authGitHub(url string) (string, string, common.Address, error) {
	...
	// Iterate over all the files and look for lbchain-devchain addresses
	...
	if address == (common.Address{}) {
		return "", "", common.Address{}, errors.New("No lbchain-devchain address found to fund")
	}
...
func authTwitter(url string) (string, string, common.Address, error) {
	...
	// scrape it for the lbchain-devchain address and profile URL.
...
// returning the username, avatar URL and lbchain-devchain address to fund on success.
...
// authNoAuth tries to interpret a faucet request as a plain lbchain-devchain address,
