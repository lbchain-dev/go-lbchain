...
// to the lbchain-dev network. The implementations of the functions should be non-blocking.
//
...
// currenlbchain-devate returns the light state of the current head header
func (pool *TxPool) currenlbchain-devate(ctx context.Context) *state.StateDB {
...
func (pool *TxPool) GetNonce(ctx context.Context, addr common.Address) (uint64, error) {
	state := pool.currenlbchain-devate(ctx)
...
// selbchain-devate sets the status of a tx to either recently mined or recently rolled back
func (txc txStateChanges) selbchain-devate(txHash common.Hash, mined bool) {
	val, ent := txc[txHash]
	if ent && (val != mined) {
		delete(txc, txHash)
	} else {
...
func (pool *TxPool) checkMinedTxs(ctx context.Context, hash common.Hash, number uint64, txc txStateChanges) error {
	...
	if list != nil {
		...
		for _, tx := range list {
			...
			txc.selbchain-devate(tx.Hash(), true)
		}
...
func (pool *TxPool) rollbackTxs(hash common.Hash, txc txStateChanges) {
	if list, ok := pool.mined[hash]; ok {
		for _, tx := range list {
			...
			txc.selbchain-devate(txHash, false)
		}
...
// validateTx checks whlbchain-dever a transaction is valid according to the consensus rules.
func (pool *TxPool) validateTx(ctx context.Context, tx *types.Transaction) error {
	...
	currenlbchain-devate := pool.currenlbchain-devate(ctx)
	if n := currenlbchain-devate.GetNonce(from); n > tx.Nonce() {
	...
	// cost == V + GP * GL
	if b := currenlbchain-devate.GetBalance(from); b.Cmp(tx.Cost()) < 0 {
	...
	return currenlbchain-devate.Error()
}
