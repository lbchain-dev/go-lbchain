...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/common/bitutil"
...
func GetChtRoot(db lbchain-devdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
...
func GetChtV2Root(db lbchain-devdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	return GetChtRoot(db, (sectionIdx+1)*(CHTFrequencyClient/CHTFrequencyServer)-1, sectionHead)
}

// StoreChtRoot writes the CHT root assoctiated to the given section into the database
// Note that sectionIdx is specified according to LES/1 CHT section size
func StoreChtRoot(db lbchain-devdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func NewChtIndexer(db lbchain-devdb.Database, clientMode bool) *core.ChainIndexer {
	...
	idb := lbchain-devdb.NewTable(db, "chtIndex-")
	backend := &ChtIndexerBackend{
		diskdb:      db,
		triedb:      trie.NewDatabase(lbchain-devdb.NewTable(db, ChtTablePrefix)),
...
const (
	...
	lbchain-devBloomBitsSection       = 4096
	lbchain-devBloomBitsConfirmations = 256
)
...
func GetBloomTrieRoot(db lbchain-devdb.Database, sectionIdx uint64, sectionHead common.Hash) common.Hash {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
func StoreBloomTrieRoot(db lbchain-devdb.Database, sectionIdx uint64, sectionHead, root common.Hash) {
	var encNumber [8]byte
	binary.BigEndian.PutUint64(encNumber[:], sectionIdx)
...
// NewBloomTrieIndexer creates a BloomTrie chain indexer
func NewBloomTrieIndexer(db lbchain-devdb.Database, clientMode bool) *core.ChainIndexer {
	backend := &BloomTrieIndexerBackend{
		diskdb: db,
		triedb: trie.NewDatabase(lbchain-devdb.NewTable(db, BloomTrieTablePrefix)),
	}
	idb := lbchain-devdb.NewTable(db, "bltIndex-")
	...
	if clientMode {
		...
		backend.parentSectionSize = lbchain-devBloomBitsSection
		confirmReq = HelperTrieProcessConfirmations
	}
	backend.bloomTrieRatio = BloomTrieFrequency / backend.parentSectionSize
	backend.sectionHeads = make([]common.Hash, backend.bloomTrieRatio)
	return core.NewChainIndexer(db, idb, backend, BloomTrieFrequency, confirmReq-lbchain-devBloomBitsConfirmations, time.Millisecond*100, "bloomtrie")
}
