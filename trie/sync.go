...
type request struct {
	...
	raw  bool        // Whlbchain-dever this is a raw entry (code) or a trie node
...
func (s *TrieSync) AddSubTrie(root common.Hash, depth int, parent common.Hash, callback LeafCallback) {
	...
	// If this sub-trie has a designated parent, link them toglbchain-dever
...
// Process injects a batch of retrieved trie nodes data, returning if somlbchain-deving
...
func (s *TrieSync) Commit(dbw lbchain-devdb.Putter) (int, error) {
...
func (s *TrieSync) children(req *request, object node) ([]*request, error) {
	// Gather all the children of the node, irrelevant whlbchain-dever known or not
	type child struct {
		node  node
		depth int
	}
