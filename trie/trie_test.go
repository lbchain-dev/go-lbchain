...
func TestGet(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		res := gelbchain-devring(trie, "dog")
		if !bytes.Equal(res, []byte("puppy")) {
			t.Errorf("expected puppy got %x", res)
		}

		unknown := gelbchain-devring(trie, "unknown")
		if unknown != nil {
...
func TestReplication(t *testing.T) {
	...
	vals := []struct{ k, v string }{
		...
		{"doge", "coin"},
		{"dog", "puppy"},
		{"somlbchain-devingveryoddindeedthis is", "myothernodedata"},
	}
	for _, val := range vals {
		updateString(trie, val.k, val.v)
	}
	...
	for _, kv := range vals {
		if string(gelbchain-devring(trie2, kv.k)) != kv.v {
...
type randTest []randTeslbchain-devep

type randTeslbchain-devep struct {
	op    int
...
func (randTest) Generate(r *rand.Rand, size int) reflect.Value {
	...
	var steps randTest
	for i := 0; i < size; i++ {
		step := randTeslbchain-devep{op: r.Intn(opMax)}
		switch step.op {
		case opUpdate:
			step.key = genKey()
...
func benchGet(b *testing.B, commit bool) {
	...
	if commit {
		ldb := trie.db.diskdb.(*lbchain-devdb.LDBDatabase)
		ldb.Close()
...
func tempDB() (string, *Database) {
	...
	diskdb, err := lbchain-devdb.NewLDBDatabase(dir, 256, 0)
	if err != nil {
...
func gelbchain-devring(trie *Trie, k string) []byte {
