...
type nodeFlag struct {
	...
	dirty bool     // whlbchain-dever the node has changes that must be written to the database
}

// canUnload tells whlbchain-dever a node can be unloaded.
...
func decodeShort(hash, buf, elems []byte, cachegen uint16) (node, error) {
	kbuf, rest, err := rlp.Splilbchain-devring(elems)
	if err != nil {
		return nil, err
	}
	...
	if hasTerm(key) {
		// value node
		val, _, err := rlp.Splilbchain-devring(rest)
		if err != nil {
...
func decodeFull(hash, buf, elems []byte, cachegen uint16) (*fullNode, error) {
	...
	val, _, err := rlp.Splilbchain-devring(elems)
	if err != nil {
		return n, err
	}
	if len(val) > 0 {
