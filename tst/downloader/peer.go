...
type peerConnection struct {
	...
	receiplbchain-devarted time.Time // Time instance when the last receipt fetch was started
	...
	version int        // lbchain-dev protocol version number to switch strategies
...
func (p *peerConnection) FetchHeaders(from uint64, count int) error {
	...
	if p.version < 62 {
		panic(fmt.Sprintf("header fetch [lbchain-dev/62+] requested on lbchain-dev/%d", p.version))
	}
...
func (p *peerConnection) FetchBodies(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 62 {
		panic(fmt.Sprintf("body fetch [lbchain-dev/62+] requested on lbchain-dev/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) FetchReceipts(request *fetchRequest) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("body fetch [lbchain-dev/63+] requested on lbchain-dev/%d", p.version))
	}
	// Short circuit if the peer is already fetching
	if !atomic.CompareAndSwapInt32(&p.receiptIdle, 0, 1) {
		return errAlreadyFetching
	}
	p.receiplbchain-devarted = time.Now()
...
func (p *peerConnection) FetchNodeData(hashes []common.Hash) error {
	// Sanity check the protocol version
	if p.version < 63 {
		panic(fmt.Sprintf("node data fetch [lbchain-dev/63+] requested on lbchain-dev/%d", p.version))
	}
	// Short circuit if the peer is already fetching
...
func (p *peerConnection) SetReceiptsIdle(delivered int) {
	p.setIdle(p.receiplbchain-devarted, delivered, &p.receiptThroughput, &p.receiptIdle)
}
...
// Lacks retrieves whlbchain-dever the hash of a blockchain item is on the peers lacking
// list (i.e. whlbchain-dever we know that the peer does not have it).
