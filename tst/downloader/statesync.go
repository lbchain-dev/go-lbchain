...
// stateReq represents a batch of state fetch requests groupped toglbchain-dever into
...
type stateReq struct {
	...
	dropped  bool                       // Flag whlbchain-dever the peer dropped off early
}
...
func (d *Downloader) runStateSync(s *stateSync) *stateSync {
	...
	for {
		...
		case req := <-timeout:
			// If the peer is already requesting somlbchain-deving else, ignore the stale timeout.
...
func (s *stateSync) loop() error {
	...
	for s.sched.Pending() > 0 {
		...
		s.assignTasks()
		// Tasks assigned, wait for somlbchain-deving to happen
		select {
		case <-newPeer:
...
func (s *stateSync) commit(force bool) error {
	if !force && s.bytesUncommitted < lbchain-devdb.IdealBatchSize {
		return nil
	}
	start := time.Now()
...
func (s *stateSync) process(req *stateReq) error {
	...
	for hash, task := range req.tasks {
		// If the node did deliver somlbchain-deving, missing items may be due to a protocol
...
// peer into the state trie, returning whlbchain-dever anything useful was written or any
// error occurred.
