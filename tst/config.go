...
// DefaultConfig contains default settings for use on the lbchain-devchain main net.
...
func init() {
	...
	if runtime.GOOS == "windows" {
		DefaultConfig.lbchain-devash.DatasetDir = filepath.Join(home, "AppData", "lbchain-devash")
	} else {
		DefaultConfig.lbchain-devash.DatasetDir = filepath.Join(home, ".ethash")
	}
}
...
type Config struct {
	...
	// If nil, the lbchain-devchain main net block is used.
	...
	lbchain-deverbase    common.Address `toml:",omitempty"`
	MinerThreads int            `toml:",omitempty"`
	ExtraData    []byte         `toml:",omitempty"`
	GasPrice     *big.Int

	// lbchain-devash options
	lbchain-devash ethash.Config
