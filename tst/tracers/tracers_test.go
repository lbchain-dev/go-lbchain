...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/tests"
)

// To generate a new callTracer test, copy paste the makeTest method below into
// a Glbchain-dev console and call it with a transaction hash you which to export.
...
var makeTest = function(tx, rewind) {
  ...
  var block   = lbchain-dev.getBlock(lbchain-dev.getTransaction(tx).blockHash);
  var genesis = lbchain-dev.getBlock(block.parentHash);
  ...
  genesis.config = admin.nodeInfo.protocols.lbchain-dev.config;
  ...
  console.log(JSON.stringify({
    ...
    input:  lbchain-dev.getRawTransaction(tx),
    result: result,
  }, null, 2));
}
*/
