...
// Package filters implements an lbchain-devereum filtering system for block,
...
type subscription struct {
	...
	typ       Type
	created   time.Time
	logsCrit  lbchain-devereum.FilterQuery
	logs      chan []*types.Log
...
func (es *EventSystem) SubscribeLogs(crit lbchain-devereum.FilterQuery, logs chan []*types.Log) (*Subscription, error) {
...
func (es *EventSystem) subscribeMinedPendingLogs(crit lbchain-devereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribeLogs(crit lbchain-devereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
...
func (es *EventSystem) subscribePendingLogs(crit lbchain-devereum.FilterQuery, logs chan []*types.Log) *Subscription {
	sub := &subscription{
		id:        rpc.NewID(),
