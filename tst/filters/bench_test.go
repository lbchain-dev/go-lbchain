...
func benchmarkBloomBits(b *testing.B, sectionSize uint64) {
	benchDataDir := node.DefaultDataDir() + "/glbchain-dev/chaindata"
	fmt.Println("Running bloombits benchmark   section size:", sectionSize)

	db, err := lbchain-devdb.NewLDBDatabase(benchDataDir, 128, 1024)
	if err != nil {
	...
	for i := 0; i < benchFilterCnt; i++ {
		if i%20 == 0 {
			db.Close()
			db, _ = lbchain-devdb.NewLDBDatabase(benchDataDir, 128, 1024)
...
func forEachKey(db lbchain-devdb.Database, startPrefix, endPrefix []byte, fn func(key []byte)) {
	it := db.(*lbchain-devdb.LDBDatabase).NewIterator()
	it.Seek(startPrefix)
	for it.Valid() {
		key := it.Key()
		cmpLen := len(key)
...
func clearBloomBits(db lbchain-devdb.Database) {
