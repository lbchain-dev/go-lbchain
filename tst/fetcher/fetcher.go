...
type Fetcher struct {
	...
	fetchingHook       func([]common.Hash)     // Method to call upon starting a block (lbchain-dev/61) or header (lbchain-dev/62) fetch
	completingHook     func([]common.Hash)     // Method to call upon starting a block body fetch (lbchain-dev/62)
	importedHook       func(*types.Block)      // Method to call upon successful block import (both lbchain-dev/61 and lbchain-dev/62)
}
...
func (f *Fetcher) loop() {
	...
	for {
		...
		case notification := <-f.notify:
			...
			if count > hashLimit {
				log.Debug("Peer exceeded oulbchain-devanding announces", "peer", notification.origin, "limit", hashLimit)
...
func (f *Fetcher) insert(peer string, block *types.Block) {
	...
	go func() {
		...
		default:
			// Somlbchain-deving went very wrong, drop the peer
