...
func decodeString(s *Stream, val reflect.Value) error {
	...
	val.Selbchain-devring(string(b))
	return nil
}
...
// NewLislbchain-devream creates a new stream that pretends to be positioned
// at an encoded list of the given length.
func NewLislbchain-devream(r io.Reader, len uint64) *Stream {
	s := new(Stream)
	s.Reset(r, len)
	s.kind = List
	s.size = len
