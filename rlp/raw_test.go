...
func TestSplitTypes(t *testing.T) {
	if _, _, err := Splilbchain-devring(unhex("C100")); err != ErrExpectedString {
		t.Errorf("Splilbchain-devring returned %q, want %q", err, ErrExpectedString)
	}
	if _, _, err := SplitList(unhex("01")); err != ErrExpectedList {
		t.Errorf("Splilbchain-devring returned %q, want %q", err, ErrExpectedList)
	}
