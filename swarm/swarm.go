...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/contracts/chequebook"
	"github.com/lbchain-devchain/go-lbchain-dev/contracts/ens"
	...
	"github.com/lbchain-devchain/go-lbchain-dev/swarm/api"
	httpapi "github.com/lbchain-devchain/go-lbchain-dev/swarm/api/http"
	"github.com/lbchain-devchain/go-lbchain-dev/swarm/fuse"
	"github.com/lbchain-devchain/go-lbchain-dev/swarm/network"
...
func NewSwarm(ctx *node.ServiceContext, backend chequebook.Backend, config *api.Config) (self *Swarm, err error) {
	...
	self.storage = storage.NewNelbchain-devore(hash, self.lstore, self.cloud, config.StoreParams)
...
func newEnsClient(endpoint string, addr common.Address, config *api.Config) (*ens.ENS, error) {
	...
	ensClient := lbchain-devclient.NewClient(client)
...
func detectEnsAddr(client *rpc.Client) (common.Address, error) {
	...
	block, err := lbchain-devclient.NewClient(client).BlockByNumber(ctx, big.NewInt(0))
	if err != nil {
		return common.Address{}, err
	}
...
* (starts nelbchain-devore level 0 api)
...
// Local swarm without nelbchain-devore
