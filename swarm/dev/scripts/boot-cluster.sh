...
#
# The cluster contains a bootnode, a glbchain-dev node and multiple swarm nodes, with
...
# static glbchain-dev configuration
Glbchain-dev_IP="192.168.33.3"
Glbchain-dev_RPC_PORT="8545"
Glbchain-dev_RPC_URL="http://${Glbchain-dev_IP}:${Glbchain-dev_RPC_PORT}"
...
# network namespaces toglbchain-dever
create_network() {
...
# start_bootnode starts a bootnode which is used to boolbchain-devrap the glbchain-dev and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a glbchain-dev node with --datadir pointing at <base-dir>/glbchain-dev
# and a single, unlocked account with password "glbchain-dev"
start_geth_node() {
  local dir="${base_dir}/glbchain-dev"
  mkdir -p "${dir}"

  local password="glbchain-dev"
  echo "${password}" > "${dir}/password"

  # create an account if necessary
  if [[ ! -e "${dir}/keystore" ]]; then
    info "creating glbchain-dev account"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get glbchain-dev account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${Glbchain-dev_IP}"
    --rpcport   "${Glbchain-dev_RPC_PORT}"
    --verbosity "6"
  )

  start_node "glbchain-dev" "${Glbchain-dev_IP}" "$(which glbchain-dev)" ${args[@]}
}
...
# bridge using a vlbchain-dev pair
create_node_network() {
  local name="$1"
  local ip="$2"
  ...
  # create the vlbchain-dev pair
  local vlbchain-dev0="vlbchain-dev${name}0"
  local vlbchain-dev1="vlbchain-dev${name}1"
  ip link add name "${vlbchain-dev0}" type vlbchain-dev peer name "${vlbchain-dev1}"

  # add one end to the bridge
  ip link set dev "${vlbchain-dev0}" master "${BRIDGE_NAME}"
  ip link set dev "${vlbchain-dev0}" up

  # add the other end to the namespace, rename it lbchain-dev0 and give it the ip
  ip link set dev "${vlbchain-dev1}" netns "${name}"
  ip netns exec "${name}" ip link set dev "${vlbchain-dev1}" name "lbchain-dev0"
  ip netns exec "${name}" ip link set dev "lbchain-dev0" up
  ip netns exec "${name}" ip address add "${ip}/24" dev "lbchain-dev0"
}
...
create_account() {
  ...
  glbchain-dev --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
