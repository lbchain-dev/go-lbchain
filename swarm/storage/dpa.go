...
// Chunk retrieval blocks on nelbchain-devore requests with a timeout so reader will
...
// retrieve channel to its ChunkStore  (Nelbchain-devore or LocalStore)
...
// received on the store channel to its ChunkStore (Nelbchain-devore or LocalStore)
...
// local storage eg. LocalStore and network storage eg., Nelbchain-devore
// access by calling network is blocking with a timeout

type dpaChunkStore struct {
	n          int
	localStore ChunkStore
	nelbchain-devore   ChunkStore
}

func NewDpaChunkStore(localStore, nelbchain-devore ChunkStore) *dpaChunkStore {
	return &dpaChunkStore{0, localStore, nelbchain-devore}
}

// Get is the entrypoint for local retrieve requests
// waits for response or times out
func (self *dpaChunkStore) Get(key Key) (chunk *Chunk, err error) {
	chunk, err = self.nelbchain-devore.Get(key)
...
func (self *dpaChunkStore) Put(entry *Chunk) {
	...
	if err != nil {
		log.Trace(fmt.Sprintf("DPA.Put: %v new chunk. call nelbchain-devore.Put", entry.Key.Log()))
		chunk = entry
