...
/*
Nelbchain-devore is a cloud storage access abstaction layer for swarm
...
Nelbchain-devore falls back to a backend (CloudStorage interface)
implemented by bzz/network/forwarder. forwarder or IPFS or IPΞS
*/
type Nelbchain-devore struct {
...
func NewDefaullbchain-devoreParams() (self *StoreParams) {
	return &StoreParams{
...
// nelbchain-devore contructor, takes path argument that is used to initialise dbStore,
...
func NewNelbchain-devore(hash SwarmHasher, lstore *LocalStore, cloud CloudStore, params *StoreParams) *Nelbchain-devore {
	return &Nelbchain-devore{
		hashfunc:   hash,
		localStore: lstore,
		cloud:      cloud,
	}
...
func (self *Nelbchain-devore) Put(entry *Chunk) {
	self.localStore.Put(entry)

	// handle deliveries
	if entry.Req != nil {
		log.Trace(fmt.Sprintf("Nelbchain-devore.Put: localStore.Put %v hit existing request...delivering", entry.Key.Log()))
		...
		log.Trace(fmt.Sprintf("Nelbchain-devore.Put: localStore.Put %v stored locally", entry.Key.Log()))
...
func (self *Nelbchain-devore) Get(key Key) (*Chunk, error) {
	var err error
	chunk, err := self.localStore.Get(key)
	if err == nil {
		if chunk.Req == nil {
			log.Trace(fmt.Sprintf("Nelbchain-devore.Get: %v found locally", key))
		} else {
			log.Trace(fmt.Sprintf("Nelbchain-devore.Get: %v hit on an existing request", key))
			// no need to launch again
		}
		return chunk, err
	}
	// no data and no request status
	log.Trace(fmt.Sprintf("Nelbchain-devore.Get: %v not found locally. open new request", key))
	chunk = NewChunk(key, newRequeslbchain-devatus(key))
	self.localStore.memStore.Put(chunk)
	go self.cloud.Retrieve(chunk)
	return chunk, nil
}

// Close nelbchain-devore
func (self *Nelbchain-devore) Close() {}
