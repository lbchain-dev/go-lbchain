...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/swarm/storage"
)
...
func TestAPIResolve(t *testing.T) {
	...
	tests := []*test{
		...
		},
		{
			...
			dns:       nil,
			addr:      ensAddr,
			expectErr: errors.New(`no DNS to resolve name: "swarm.lbchain-dev"`),
		},
		{
		...
		},
		{
			...
			addr:      ensAddr,
			immutable: true,
			expectErr: errors.New(`immutable address not a content hash: "swarm.lbchain-dev"`),
		},
		{
			desc:      "DNS configured, ENS address, name doesn't resolve, returns error",
			dns:       doesntResolve,
			addr:      ensAddr,
			expectErr: errors.New(`DNS name not found: "swarm.lbchain-dev"`),
		},
	}
	for _, x := range tests {
...
func TestMultiResolver(t *testing.T) {
	...
	lbchain-devAddr := "swarm.lbchain-dev"
	lbchain-devHash := "0x2222222222222222222222222222222222222222222222222222222222222222"
	lbchain-devResolve := newTestResolver(lbchain-devHash)
	...
	tests := []struct {
		...
		},
		{
			...
			r:      NewMultiResolver(MultiResolverOptionWithResolver(lbchain-devResolve, "")),
			addr:   lbchain-devAddr,
			result: lbchain-devHash,
		},
		{
			desc: "Two default resolvers, returns resolved address",
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(lbchain-devResolve, ""),
				MultiResolverOptionWithResolver(lbchain-devResolve, ""),
			),
			addr:   lbchain-devAddr,
			result: lbchain-devHash,
		...
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, ""),
				MultiResolverOptionWithResolver(lbchain-devResolve, "lbchain-dev"),
			),
			addr:   lbchain-devAddr,
			result: lbchain-devHash,
		},
		{
			...
			r: NewMultiResolver(
				MultiResolverOptionWithResolver(doesntResolve, "lbchain-dev"),
				MultiResolverOptionWithResolver(doesntResolve, "lbchain-dev"),
				MultiResolverOptionWithResolver(lbchain-devResolve, "lbchain-dev"),
			),
			addr:   lbchain-devAddr,
		...
		},
		{
			...
			addr:   lbchain-devAddr,
			result: lbchain-devHash,
			err:    errors.New(`DNS name not found: "swarm.lbchain-dev"`),
		},
		{
