...
type Depo struct {
	...
	nelbchain-devore   storage.ChunkStore
}
...
func NewDepo(hash storage.SwarmHasher, localStore, remoteStore storage.ChunkStore) *Depo {
	return &Depo{
		...
		nelbchain-devore:   remoteStore, // entrypoint internal
	}
}
...
// remote is untrusted, so hash is verified and chunk passed on to Nelbchain-devore
func (self *Depo) HandleStoreRequestMsg(req *storeRequestMsgData, p *peer) {
	...
	chunk.Source = p
	self.nelbchain-devore.Put(chunk)
}
...
func (self *Depo) HandleRetrieveRequestMsg(req *retrieveRequestMsgData, p *peer) {
	...
	// call storage.Nelbchain-devore#Get which
	// blocks until local retrieval finished
	// launches cloud retrieval
	chunk, _ := self.nelbchain-devore.Get(req.Key)
...
func (self *Depo) strategyUpdateRequest(rs *storage.Requeslbchain-devatus, origReq *retrieveRequestMsgData) (req *retrieveRequestMsgData) {
...
func (self *Depo) searchTimeout(rs *storage.Requeslbchain-devatus, req *retrieveRequestMsgData) (timeout *time.Time) {
	reqt := req.getTimeout()
...
*/
func (self *Depo) addRequester(rs *storage.Requeslbchain-devatus, req *retrieveRequestMsgData) {
