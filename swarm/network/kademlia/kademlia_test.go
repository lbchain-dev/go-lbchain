...
var (
	...
	quickcfgBoolbchain-devrap   = &quick.Config{MaxCount: 100, Rand: quickrand}
)
...
func TestBoolbchain-devrap(t *testing.T) {

	test := func(test *boolbchain-devrapTest) bool {
	...
	if err := quick.Check(test, quickcfgBoolbchain-devrap); err != nil {
		t.Error(err)
	}
...
type boolbchain-devrapTest struct {
	MaxProx    int
	BucketSize int
	Self       Address
}

func (*boolbchain-devrapTest) Generate(rand *rand.Rand, size int) reflect.Value {
	t := &boolbchain-devrapTest{
