...
var (
	...
	procGelbchain-devdHandle                  = kernel32.NewProc("Gelbchain-devdHandle")
...
func NewLiner() *State {
	var s State
	hIn, _, _ := procGelbchain-devdHandle.Call(uintptr(std_input_handle))
	s.handle = syscall.Handle(hIn)
	hOut, _, _ := procGelbchain-devdHandle.Call(uintptr(std_output_handle))
...
func (mode inputMode) ApplyMode() error {
	hIn, _, err := procGelbchain-devdHandle.Call(uintptr(std_input_handle))
