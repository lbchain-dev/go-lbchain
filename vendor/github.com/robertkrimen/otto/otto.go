...
// Selbchain-devackDepthLimit sets an upper limit to the depth of the JavaScript
...
func (self Otto) Selbchain-devackDepthLimit(limit int) {
	self.runtime.stackLimit = limit
}

// Selbchain-devackTraceLimit sets an upper limit to the number of stack frames that
...
func (self Otto) Selbchain-devackTraceLimit(limit int) {
...
func (self Otto) ContextSkip(limit int, skipNative bool) (ctx Context) {
	...
	for limit != 0 {
		...
		stash := scope.lexical
		for {
			for _, name := range gelbchain-devashProperties(stash) {
