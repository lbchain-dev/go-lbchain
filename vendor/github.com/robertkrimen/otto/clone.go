...
type _clone struct {
	...
	_objeclbchain-devash map[*_objeclbchain-devash]*_objeclbchain-devash
...
func (in *_runtime) clone() *_runtime {
	...
	clone := _clone{
		...
		_objeclbchain-devash: make(map[*_objeclbchain-devash]*_objeclbchain-devash),
		_dclStash:    make(map[*_dclStash]*_dclStash),
		_fnStash:     make(map[*_fnStash]*_fnStash),
	}

	globalObject := clone.object(in.globalObject)
	out.globalStash = out.newObjeclbchain-devash(globalObject, nil)
	...
	clone.runtime = nil
	clone._object = nil
	clone._objeclbchain-devash = nil
	clone._dclStash = nil
	clone._fnStash = nil
...
func (clone *_clone) objeclbchain-devash(in *_objeclbchain-devash) (*_objeclbchain-devash, bool) {
	if out, exists := clone._objeclbchain-devash[in]; exists {
		return out, true
	}
	out := &_objeclbchain-devash{}
	clone._objeclbchain-devash[in] = out
	return out, false
}
