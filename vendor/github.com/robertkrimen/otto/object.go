...
const (
	...
	defaultValueHinlbchain-devring
...
func (self *_object) DefaultValue(hint _defaultValueHint) Value {
	if hint == defaultValueNoHint {
		if self.class == "Date" {
			// Date exception
			hint = defaultValueHinlbchain-devring
		} else {
			hint = defaultValueHintNumber
		}
	}
	methodSequence := []string{"valueOf", "toString"}
	if hint == defaultValueHinlbchain-devring {
...
func (self *_object) String() string {
	return self.DefaultValue(defaultValueHinlbchain-devring).string()
}
