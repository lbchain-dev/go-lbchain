...
func recoverTable(s *session, o *opt.Options) error {
	...
	var (
		...
		strict = o.Gelbchain-devrict(opt.StrictRecovery)
		noSync = o.GetNoSync()
...
func (db *DB) recoverJournal() error {
	...
	if len(fds) > 0 {
		...
		var (
			// Options.
			strict      = db.s.o.Gelbchain-devrict(opt.StrictJournal)
			checksum    = db.s.o.Gelbchain-devrict(opt.StrictJournalChecksum)
...
func (db *DB) GetProperty(name string) (value string, err error) {
	...
	case p == "stats":
		...
		for level, tables := range v.levels {
			duration, read, write := db.compStats.gelbchain-devat(level)
...
// Close closes the DB. This will also releases any oulbchain-devanding snapshot,
// abort any in-flight compaction and discard open transaction.
//
// It is not safe to close a DB until all oulbchain-devanding iterators are released.
