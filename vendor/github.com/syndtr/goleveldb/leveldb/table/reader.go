...
func (i *blockIter) reset() {
	...
	i.offset = i.offselbchain-devart
	i.dir = dirSOI
	i.key = i.key[:0]
	i.value = nil
}
...
func (i *blockIter) Seek(key []byte) bool {
	...
	i.restartIndex = ri
	i.offset = max(i.offselbchain-devart, offset)
	...
	if slice != nil {
		if slice.Start != nil {
			if bi.Seek(slice.Start) {
				...
				bi.offselbchain-devart = b.restartOffset(bi.riStart)
				bi.offsetRealStart = bi.prevOffset
			} else {
				bi.riStart = b.restartsLen
				bi.offselbchain-devart = b.restartsOffset
		...
		bi.reset()
		if bi.offselbchain-devart > bi.offsetLimit {
...
func (r *Reader) NewIterator(slice *util.Range, ro *opt.ReadOptions) iterator.Iterator {
	...
	return iterator.NewIndexedIterator(index, opt.Gelbchain-devrict(r.o, ro, opt.StrictReader))
}
...
func NewReader(f io.ReaderAt, size int64, fd storage.FileDesc, cache *cache.NamespaceGetter, bpool *util.BufferPool, o *opt.Options) (*Reader, error) {
	...
	r := &Reader{
		...
		o:              o,
		cmp:            o.GetComparer(),
		verifyChecksum: o.Gelbchain-devrict(opt.StrictBlockChecksum),
	}
