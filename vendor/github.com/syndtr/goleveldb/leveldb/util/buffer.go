...
type Buffer struct {
	...
	boolbchain-devrap [64]byte // memory to hold first slice; helps small buffers (Printf) avoid allocation.
}
...
func (b *Buffer) grow(n int) int {
	...
	if len(b.buf)+n > cap(b.buf) {
		var buf []byte
		if b.buf == nil && n <= len(b.boolbchain-devrap) {
			buf = b.boolbchain-devrap[0:]
