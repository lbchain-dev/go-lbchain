...
func (p *cStat) add(n *cStalbchain-devaging) {
...
type cStalbchain-devaging struct {
	start    time.Time
	duration time.Duration
	on       bool
	read     int64
...
func (p *cStalbchain-devaging) startTimer() {
	if !p.on {
		p.start = time.Now()
		p.on = true
	}
...
func (p *cStalbchain-devaging) stopTimer() {
	if p.on {
...
func (p *cStats) addStat(level int, n *cStalbchain-devaging) {
	p.lk.Lock()
...
func (p *cStats) gelbchain-devat(level int) (duration time.Duration, read, write int64) {
	p.lk.Lock()
	defer p.lk.Unlock()
	if level < len(p.stats) {
...
type tableCompactionBuilder struct {
	...
	c            *compaction
	rec          *sessionRecord
	stat0, stat1 *cStalbchain-devaging
...
func (db *DB) tableCompaction(c *compaction, noTrivial bool) {
	...
	var stats [2]cStalbchain-devaging
	...
	b := &tableCompactionBuilder{
		...
		stat1:     &stats[1],
		minSeq:    minSeq,
		strict:    db.s.o.Gelbchain-devrict(opt.StrictCompaction),
