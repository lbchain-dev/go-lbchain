...
var DefaultEvlbchain-devream = NewEvlbchain-devream()

type Evlbchain-devream struct {
	sync.RWMutex
...
func NewEvlbchain-devream() *Evlbchain-devream {
	return &Evlbchain-devream{
...
func (es *Evlbchain-devream) Init() {
...
func (es *Evlbchain-devream) Merge(name string, ec chan Event) {
	es.Lock()
	defer es.Unlock()
...
func (es *Evlbchain-devream) Handle(path string, handler func(Event)) {
...
func (es *Evlbchain-devream) ResetHandlers() {
...
func (es *Evlbchain-devream) match(path string) string {
	return findMatch(es.Handlers, path)
}

func (es *Evlbchain-devream) Hook(f func(Event)) {
	es.hook = f
}

func (es *Evlbchain-devream) Loop() {
...
func (es *Evlbchain-devream) StopLoop() {
	go func() {
		e := Event{
			Path: "/sig/stoploop",
		}
...
func Merge(name string, ec chan Event) {
	DefaultEvlbchain-devream.Merge(name, ec)
}

func Handle(path string, handler func(Event)) {
	DefaultEvlbchain-devream.Handle(path, handler)
}

func Loop() {
	DefaultEvlbchain-devream.Loop()
}

func StopLoop() {
	DefaultEvlbchain-devream.StopLoop()
}
