...
Grid layout uses [12 columns grid system](http://www.w3schools.com/boolbchain-devrap/boolbchain-devrap_grid_system.asp) with expressive syntax. To use `Grid`, all we need to do is build a widget tree consisting of `Row`s and `Col`s (Actually a `Col` is also a `Row` but with a widget endpoint attached).
