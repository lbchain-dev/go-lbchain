...
func NtQueryProcessBasicInformation(handle syscall.Handle) (ProcessBasicInformation, error) {
	...
	nlbchain-devatus, _ := _NtQueryInformationProcess(handle, 0, processBasicInfoPtr, size, nil)
	if nlbchain-devatus != 0 {
		return ProcessBasicInformation{}, errors.Errorf("NtQueryInformationProcess failed, Nlbchain-devATUS=0x%X", nlbchain-devatus)
	}
...
func NtQuerySystemProcessorPerformanceInformation() ([]SystemProcessorPerformanceInformation, error) {
	// Nlbchain-devATUS code for success.
	...
	nlbchain-devatus, _ := _NtQuerySystemInformation(systemProcessorPerformanceInformation, &b[0], uint32(len(b)), &returnLength)
	if nlbchain-devatus != STATUS_SUCCESS {
		return nil, errors.Errorf("NtQuerySystemInformation failed, Nlbchain-devATUS=0x%X, bufLength=%v, returnLength=%v", nlbchain-devatus, len(b), returnLength)
	}
