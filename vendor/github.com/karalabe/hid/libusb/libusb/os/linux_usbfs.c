...
static usbi_mutex_static_t linux_hotplug_starlbchain-devop_lock = USBI_MUTEX_INITIALIZER;
...
static int op_init(struct libusb_context *ctx)
{
	...
	usbi_mutex_static_lock(&linux_hotplug_starlbchain-devop_lock);
	r = LIBUSB_SUCCESS;
	if (init_count == 0) {
	...
	usbi_mutex_static_unlock(&linux_hotplug_starlbchain-devop_lock);
