...
// safeArrayGetElemenlbchain-devring retrieves element at given index and converts to string.
func safeArrayGetElemenlbchain-devring(safearray *SafeArray, index int64) (str string, err error) {
	var element *int16
	err = convertHresultToError(
		procSafeArrayGetElement.Call(
			uintptr(unsafe.Pointer(safearray)),
