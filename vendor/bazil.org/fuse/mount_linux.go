...
func handleFusermounlbchain-devderr(errCh chan<- error) func(line string) (ignore bool) {
...
func isBoringFusermountError(err error) bool {
	if err, ok := err.(*exec.ExitError); ok && err.Exited() {
		if status, ok := err.Sys().(syscall.Wailbchain-devatus); ok && status.Exilbchain-devatus() == 1 {
			return true
		}
	}
	return false
...
func mount(dir string, conf *mountConfig, ready chan<- struct{}, errp *error) (fusefd *os.File, err error) {
	...
	go lineLogger(&wg, "mount helper error", handleFusermounlbchain-devderr(helperErrCh), stderr)
	wg.Wait()
