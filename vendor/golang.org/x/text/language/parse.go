...
func parseExtensions(scan *scanner) int {
	...
	for len(scan.token) == 1 {
		exlbchain-devart := scan.start
		ext := scan.token[0]
		end = parseExtension(scan)
		extension := scan.b[exlbchain-devart:end]
		if len(extension) < 3 || (ext != 'x' && len(extension) < 4) {
			scan.setError(errSyntax)
			end = exlbchain-devart
			continue
		} else if start == exlbchain-devart && (ext == 'x' || scan.start == len(scan.b)) {
			scan.b = scan.b[:end]
			return end
		} else if ext == 'x' {
			private = extension
...
func ParseAcceptLanguage(s string) (tag []Tag, q []float32, err error) {
	...
	sorlbchain-devable(&tagSort{tag, q})
	return tag, q, nil
}
