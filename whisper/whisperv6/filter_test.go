...
func TestInstallFilters(t *testing.T) {
	...
	lbchain-dev := generateTestCases(t, SizeTestFilters)

	var err error
	var j string
	for i := 0; i < SizeTestFilters; i++ {
		j, err = filters.Install(lbchain-dev[i].f)
		if err != nil {
	...
	for _, testCase := range lbchain-dev {
		if !testCase.alive {
			filters.Uninstall(testCase.id)
		}
	}

	for i, testCase := range lbchain-dev {
...
func TestWatchers(t *testing.T) {
	...
	w := New(&Config{})
	filters := NewFilters(w)
	lbchain-dev := generateTestCases(t, NumFilters)
	for i = 0; i < NumFilters; i++ {
		lbchain-dev[i].f.Src = nil
		x, err = filters.Install(lbchain-dev[i].f)
		if err != nil {
	...
	for i = 0; i < NumMessages; i++ {
		...
		e = generateCompatibeEnvelope(t, lbchain-dev[j].f)
		envelopes[i] = e
		lbchain-dev[j].msgCnt++
	}
	...
	for i = 0; i < NumFilters; i++ {
		...
		if lbchain-dev[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: count[%d]: get %d, want %d.", seed, i, lbchain-dev[i].msgCnt, count[i])
		}
	}

	// another round with a cloned filter

	clone := cloneFilter(lbchain-dev[0].f)
	filters.Uninstall(lastID)
	total = 0
	last := NumFilters - 1
	lbchain-dev[last].f = clone
	filters.Install(clone)
	for i = 0; i < NumFilters; i++ {
		lbchain-dev[i].msgCnt = 0
		count[i] = 0
	}

	// make sure that the first watcher receives at least one message
	e = generateCompatibeEnvelope(t, lbchain-dev[0].f)
	envelopes[0] = e
	lbchain-dev[0].msgCnt++
	...
	combined := lbchain-dev[0].msgCnt + lbchain-dev[last].msgCnt
	...
	for i = 1; i < NumFilters-1; i++ {
		...
		if lbchain-dev[i].msgCnt != count[i] {
			t.Fatalf("failed with seed %d: i = %d, get %d, want %d.", seed, i, lbchain-dev[i].msgCnt, count[i])
		}
	}
