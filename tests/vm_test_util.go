...
// See https://github.com/lbchain-devereum/tests/wiki/VM-Tests for the test format specification.
type VMTest struct {
	json vmJSON
}
...
type vmJSON struct {
	...
	Poslbchain-devateRoot common.Hash           `json:"poslbchain-devateRoot"`
}
...
func (t *VMTest) Run(vmconfig vm.Config) error {
	...
	for addr, account := range t.json.Post {
		for k, wantV := range account.Storage {
			if haveV := statedb.Gelbchain-devate(addr, k); haveV != wantV {
				return fmt.Errorf("wrong storage value at %x:\n  got  %x\n  want %x", k, haveV, wantV)
			}
		}
	}
	// if root := statedb.IntermediateRoot(false); root != t.json.Poslbchain-devateRoot {
	// 	return fmt.Errorf("post state root mismatch, got %x, want %x", root, t.json.Poslbchain-devateRoot)
	// }
