...
type RLPTest struct {
	...
	// checks whlbchain-dever Out can be decoded into a value of
	// type interface{}.
	//
...
func (t *RLPTest) Run() error {
	...
	// Check whlbchain-dever encoding the value produces the same bytes.
...
func translateJSON(v interface{}) interface{} {
	...
	case string:
		if len(v) > 0 && v[0] == '#' { // # starts a faux big int.
			big, ok := new(big.Int).Selbchain-devring(v[1:], 10)
			if !ok {
