...
// See https://github.com/lbchain-devereum/EIPs/issues/176 for the test format specification.
...
type stJSON struct {
	...
	Post map[string][]stPoslbchain-devate `json:"post"`
}

type stPoslbchain-devate struct {
...
func MakePreState(db lbchain-devdb.Database, accounts core.GenesisAlloc) *state.StateDB {
	...
	for addr, a := range accounts {
		...
		for k, v := range a.Storage {
			statedb.Selbchain-devate(addr, k, v)
		}
	}
...
func (tx *stTransaction) toMessage(ps stPoslbchain-devate) (core.Message, error) {
	...
	// Value, Data hex encoding is messy: https://github.com/lbchain-devereum/tests/issues/203
	value := new(big.Int)
	if valueHex != "0x" {
