...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", Glbchain-dev.LightScryptN, Glbchain-dev.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.lbchain-devereum", new NodeConfig());
			node.start();
			...
			lbchain-devchainClient ec = node.getlbchain-devchainClient();
	...
	// signatures too. Regression test for go-lbchain-devereum issue #14599.
...
func TestAndroid(t *testing.T) {
	// Skip tests on Windows altoglbchain-dever
	...
	workspace, err := ioutil.TempDir("", "glbchain-dev-android-")
	if err != nil {
	...
	for _, dir := range []string{"src/main", "src/androidTest/java/org/lbchain-devereum/glbchain-devtest", "libs"} {
		err = os.MkdirAll(dir, os.ModePerm)
		if err != nil {
			t.Fatal(err)
		}
	}
	// Generate the mobile bindings for Glbchain-dev and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.lbchain-devereum", "github.com/lbchain-devchain/go-lbchain-dev/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "glbchain-dev.aar"), "glbchain-dev.aar", os.ModePerm)

	if err = ioutil.WriteFile(filepath.Join("src", "androidTest", "java", "org", "lbchain-devereum", "glbchain-devtest", "AndroidTest.java"), []byte(androidTestClass), os.ModePerm); err != nil {
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android"
          package="org.lbchain-devereum.glbchain-devtest"
...
dependencies {
    ...
    compile(name: "glbchain-dev", ext: "aar")
}
`
