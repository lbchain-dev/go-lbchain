...
// Contains all the wrappers from the go-lbchain-devereum root package.
...
type Subscription struct {
	sub lbchain-devereum.Subscription
}
...
// the lbchain-devchain network.
type SyncProgress struct {
	progress lbchain-devereum.SyncProgress
}

func (p *SyncProgress) Gelbchain-devartingBlock() int64 { return int64(p.progress.StartingBlock) }
...
type FilterQuery struct {
	query lbchain-devereum.FilterQuery
}
