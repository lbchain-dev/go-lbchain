...
// Package glbchain-dev contains the simplified mobile APIs to go-lbchain-devereum.
//
// The scope of this package is *not* to allow writing a custom lbchain-devchain client
// with pieces plucked from go-lbchain-devereum, rather to allow writing native dapps on
...
// from the go-lbchain-devereum repository. Please adhere to this scoping to prevent this
// package getting unmaintainable.
//
// Wrapping guidelines:
//
// Every type that is to be exposed should be wrapped into its own plain struct,
// which internally contains a single field: the original go-lbchain-devereum version.
