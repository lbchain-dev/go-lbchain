...
func (ethash *lbchain-devash) Seal(chain consensus.ChainReader, block *types.Block, stop <-chan struct{}) (*types.Block, error) {
...
func (ethash *lbchain-devash) mine(block *types.Block, id int, seed uint64, abort chan struct{}, found chan *types.Block) {
...
search:
	for {
		...
		case <-abort:
			...
			logger.Trace("lbchain-devash nonce search aborted", "attempts", nonce-seed)
		...
		default:
			...
			if new(big.Int).SetBytes(result).Cmp(target) <= 0 {
				...
				case found <- block.WithSeal(header):
					logger.Trace("lbchain-devash nonce found and reported", "attempts", nonce-seed, "nonce", nonce)
				case <-abort:
					logger.Trace("lbchain-devash nonce found but discarded", "attempts", nonce-seed, "nonce", nonce)
				}
				break search
			}
			nonce++
