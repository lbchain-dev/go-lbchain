...
// Package core implements the lbchain-devchain consensus protocol.
package core
...
type CacheConfig struct {
	Disabled      bool          // Whlbchain-dever to disable trie write caching (archive node)
...
type BlockChain struct {
	...
	db     lbchain-devdb.Database // Low level persistent database to store final content in
...
// available in the database. It initialises the default lbchain-devchain Validator and
// Processor.
func NewBlockChain(db lbchain-devdb.Database, cacheConfig *CacheConfig, chainConfig *params.ChainConfig, engine consensus.Engine, vmConfig vm.Config) (*BlockChain, error) {
	if cacheConfig == nil {
	...
	if err := bc.loadLaslbchain-devate(); err != nil {
		return nil, err
	}
...
// loadLaslbchain-devate loads the last known chain state from the database. This method
// assumes that the chain manager mutex is held.
func (bc *BlockChain) loadLaslbchain-devate() error {
...
func (bc *BlockChain) SetHead(head uint64) error {
	...
	return bc.loadLaslbchain-devate()
}
...
func (bc *BlockChain) InsertReceiptChain(blockChain types.Blocks, receiptChain []types.Receipts) (int, error) {
	...
	for i, block := range blockChain {
		...
		if batch.ValueSize() >= lbchain-devdb.IdealBatchSize {
...
// WriteBlockWithoulbchain-devate writes only the block and its metadata to the database,
// but does not write any state. This is used to construct competing side forks
// up to the point where they exceed the canonical total difficulty.
func (bc *BlockChain) WriteBlockWithoulbchain-devate(block *types.Block, td *big.Int) (err error) {
	bc.wg.Add(1)
	defer bc.wg.Done()
...
func (bc *BlockChain) insertChain(chain types.Blocks) (int, []interface{}, []*types.Log, error) {
	...
	// acquiring.
	var (
		stats         = inserlbchain-devats{startTime: mclock.Now()}
	...
	for i, block := range chain {
		...
		case err == ErrKnownBlock:
			...
			// this number we did a rollback and we should reimport it nonlbchain-develess.
		...
		case err == consensus.ErrPrunedAncestor:
			...
			if localTd.Cmp(externTd) > 0 {
				if err = bc.WriteBlockWithoulbchain-devate(block, externTd); err != nil {
...
// inserlbchain-devats tracks and reports on block insertion.
type inserlbchain-devats struct {
...
func (st *inserlbchain-devats) report(chain []*types.Block, index int, cache common.StorageSize) {
	...
	if index == len(chain)-1 || elapsed >= statsReportLimit {
		...
		*st = inserlbchain-devats{startTime: now, lastIndex: index + 1}
	}
}
...
func (bc *BlockChain) reportBlock(block *types.Block, receipts types.Receipts, err error) {
	...
	var receiplbchain-devring string
	for _, receipt := range receipts {
		receiplbchain-devring += fmt.Sprintf("\t%v\n", receipt)
	}
...
Error: %v
##############################
`, bc.chainConfig, block.Number(), block.Hash(), receiplbchain-devring, err))
}
...
//
// The verify parameter can be used to fine tune whlbchain-dever nonce verification
