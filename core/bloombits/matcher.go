...
type Matcher struct {
	...
	running uint32 // Atomic flag whlbchain-dever a session is live or not
}
...
// that address/topic, and binary AND-ing those vectors toglbchain-dever.
	...
	go func() {
		...
		for {
			...
			case subres, ok := <-process:
				...
				// Gather all the sub-results and merge them toglbchain-dever
				var orVector []byte
...
// the reuested retrieval queue to be serviced toglbchain-dever with other sessions.
//
...
func (s *MatcherSession) Multiplex(batch int, wait time.Duration, mux chan chan *Retrieval) {
	for {
		...
		case mux <- request:
			// Retrieval accepted, somlbchain-deving must arrive before we're aborting
