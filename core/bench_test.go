...
// genTxRing returns a block generator that sends lbchain-dever in a ring
...
func benchInsertChain(b *testing.B, disk bool, gen func(int, *BlockGen)) {
	...
	var db lbchain-devdb.Database
	if !disk {
		db, _ = lbchain-devdb.NewMemDatabase()
	} else {
		dir, err := ioutil.TempDir("", "lbchain-dev-core-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		defer os.RemoveAll(dir)
		db, err = lbchain-devdb.NewLDBDatabase(dir, 128, 128)
		if err != nil {
...
// into a database.
func makeChainForBench(db lbchain-devdb.Database, full bool, count uint64) {
...
func benchWriteChain(b *testing.B, full bool, count uint64) {
	for i := 0; i < b.N; i++ {
		dir, err := ioutil.TempDir("", "lbchain-dev-chain-bench")
		if err != nil {
			b.Fatalf("cannot create temporary directory: %v", err)
		}
		db, err := lbchain-devdb.NewLDBDatabase(dir, 128, 1024)
		if err != nil {
