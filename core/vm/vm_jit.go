...
// More: https://github.com/lbchain-devereum/evmjit
#cgo LDFLAGS: -levmjit
*/
import "C"
...
func env_sstore(vmPtr unsafe.Pointer, indexPtr unsafe.Pointer, valuePtr unsafe.Pointer) {
	...
	value = trim(value)
	if len(value) == 0 {
		prevValue := vm.env.State().Gelbchain-devate(vm.me.Address(), index)
		if len(prevValue) != 0 {
			vm.Env().State().Refund(vm.callerAddr, GasSStoreRefund)
		}
	}

	vm.env.State().Selbchain-devate(vm.me.Address(), index, value)
}
...
func env_sload(vmPtr unsafe.Pointer, indexPtr unsafe.Pointer, resultPtr unsafe.Pointer) {
	vm := (*JitVm)(vmPtr)
	index := llvm2hash(bswap((*i256)(indexPtr)))
	value := vm.env.State().Gelbchain-devate(vm.me.Address(), index)
	result := (*i256)(resultPtr)
