...
type operation struct {
	...
	halts   bool // indicates whlbchain-dever the operation should halt further execution
	jumps   bool // indicates whlbchain-dever the program counter should not increment
	writes  bool // determines whlbchain-dever this a state modifying operation
	valid   bool // indication whlbchain-dever the retrieved operation is valid and known
	reverts bool // determines whlbchain-dever the operation reverts state (implicitly halts)
	returns bool // determines whlbchain-dever the operations sets the return data content
}
...
func NewConstantinopleInstructionSet() [256]operation {
	...
	instructionSet[SHL] = operation{
		execute:       opSHL,
		gasCost:       constGasFunc(GasFasteslbchain-devep),
...
func NewFrontierInstructionSet() [256]operation {
	return [256]operation{
		...
		MUL: {
			execute:       opMul,
			gasCost:       constGasFunc(GasFaslbchain-devep),
			validateStack: makeStackFunc(2, 1),
			valid:         true,
		},
		SUB: {
		...
		BLOCKHASH: {
			execute:       opBlockhash,
			gasCost:       constGasFunc(GasExlbchain-devep),
			validateStack: makeStackFunc(1, 1),
			valid:         true,
		},
		COINBASE: {
