...
func (s *StateSuite) SetUpTest(c *checker.C) {
	s.db, _ = lbchain-devdb.NewMemDatabase()
...
func (s *StateSuite) TestNull(c *checker.C) {
	...
	s.state.Selbchain-devate(address, common.Hash{}, value)
	s.state.Commit(false)
	value = s.state.Gelbchain-devate(address, common.Hash{})
...
func (s *StateSuite) TestSnapshot(c *checker.C) {
	...
	s.state.Selbchain-devate(stateobjaddr, storageaddr, data1)
	// get snapshot of current state
	snapshot := s.state.Snapshot()

	// set new state object value
	s.state.Selbchain-devate(stateobjaddr, storageaddr, data2)
	// restore snapshot
	s.state.RevertToSnapshot(snapshot)

	// get state storage value
	res := s.state.Gelbchain-devate(stateobjaddr, storageaddr)
...
func TestSnapshot2(t *testing.T) {
	...
	state.Selbchain-devate(stateobjaddr0, storageaddr, data0)
	state.Selbchain-devate(stateobjaddr1, storageaddr, data1)

	// db, trie are already non-empty values
	so0 := state.gelbchain-devateObject(stateobjaddr0)
	so0.SetBalance(big.NewInt(42))
	so0.SetNonce(43)
	so0.SetCode(crypto.Keccak256Hash([]byte{'c', 'a', 'f', 'e'}), []byte{'c', 'a', 'f', 'e'})
	so0.suicided = false
	so0.deleted = false
	state.selbchain-devateObject(so0)

	root, _ := state.Commit(false)
	state.Reset(root)

	// and one with deleted == true
	so1 := state.gelbchain-devateObject(stateobjaddr1)
	so1.SetBalance(big.NewInt(52))
	so1.SetNonce(53)
	so1.SetCode(crypto.Keccak256Hash([]byte{'c', 'a', 'f', 'e', '2'}), []byte{'c', 'a', 'f', 'e', '2'})
	so1.suicided = true
	so1.deleted = true
	state.selbchain-devateObject(so1)
	...
	so0Restored := state.gelbchain-devateObject(stateobjaddr0)
	// Update lazily-loaded values before comparing.
	so0Restored.Gelbchain-devate(state.db, storageaddr)
	so0Restored.Code(state.db)
	// non-deleted is equal (restored)
	compareStateObjects(so0Restored, so0, t)

	// deleted should be nil, both before and after restore of state copy
	so1Restored := state.gelbchain-devateObject(stateobjaddr1)
	if so1Restored != nil {
