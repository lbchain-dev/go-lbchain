...
func TestUpdateLeaks(t *testing.T) {
	...
	for i := byte(0); i < 255; i++ {
		...
		if i%2 == 0 {
			state.Selbchain-devate(addr, common.BytesToHash([]byte{i, i, i}), common.BytesToHash([]byte{i, i, i, i}))
		}
		if i%3 == 0 {
...
func TestIntermediateLeaks(t *testing.T) {
	...
	transDb, _ := lbchain-devdb.NewMemDatabase()
	finalDb, _ := lbchain-devdb.NewMemDatabase()
	...
	modify := func(state *StateDB, addr common.Address, i, tweak byte) {
		...
		if i%2 == 0 {
			state.Selbchain-devate(addr, common.Hash{i, i, i, 0}, common.Hash{})
			state.Selbchain-devate(addr, common.Hash{i, i, i, tweak}, common.Hash{i, i, i, i, tweak})
		}
		if i%3 == 0 {
...
// https://github.com/lbchain-devchain/go-lbchain-dev/pull/15549.
...
func newTestAction(addr common.Address, r *rand.Rand) testAction {
	actions := []testAction{
		...
		{
			name: "Selbchain-devate",
			fn: func(a testAction, s *StateDB) {
				var key, val common.Hash
				binary.BigEndian.PutUint16(key[:], uint16(a.args[0]))
				binary.BigEndian.PutUint16(val[:], uint16(a.args[1]))
				s.Selbchain-devate(addr, key, val)
			},
			args: make([]int64, 2),
		},
		{
...
func (test *snapshotTest) checkEqual(state, checkstate *StateDB) error {
	for _, addr := range test.addrs {
		...
		// Check storage.
		if obj := state.gelbchain-devateObject(addr); obj != nil {
			state.ForEachStorage(addr, func(key, val common.Hash) bool {
				return checkeq("Gelbchain-devate("+key.Hex()+")", val, checkstate.Gelbchain-devate(addr, key))
			})
			checkstate.ForEachStorage(addr, func(key, checkval common.Hash) bool {
				return checkeq("Gelbchain-devate("+key.Hex()+")", state.Gelbchain-devate(addr, key), checkval)
			})
		}
		if err != nil {
			return err
