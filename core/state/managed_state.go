...
// Selbchain-devate sets the backing layer of the managed state
func (ms *ManagedState) Selbchain-devate(statedb *StateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	ms.StateDB = statedb
}
...
// HasAccount returns whlbchain-dever the given address is managed or not
...
func (ms *ManagedState) getAccount(addr common.Address) *account {
	if account, ok := ms.accounts[addr]; !ok {
		...
		so := ms.StateDB.gelbchain-devateObject(addr)
