...
var (
	...
	configPrefix   = []byte("lbchain-devereum-config-") // config prefix for the db
...
func WriteCanonicalHash(db lbchain-devdb.Putter, hash common.Hash, number uint64) error {
...
func WriteHeadHeaderHash(db lbchain-devdb.Putter, hash common.Hash) error {
...
func WriteHeadBlockHash(db lbchain-devdb.Putter, hash common.Hash) error {
...
func WriteHeadFastBlockHash(db lbchain-devdb.Putter, hash common.Hash) error {
...
func WriteTrieSyncProgress(db lbchain-devdb.Putter, count uint64) error {
...
func WriteHeader(db lbchain-devdb.Putter, header *types.Header) error {
...
func WriteBody(db lbchain-devdb.Putter, hash common.Hash, number uint64, body *types.Body) error {
...
func WriteBodyRLP(db lbchain-devdb.Putter, hash common.Hash, number uint64, rlp rlp.RawValue) error {
...
func WriteTd(db lbchain-devdb.Putter, hash common.Hash, number uint64, td *big.Int) error {
...
func WriteBlock(db lbchain-devdb.Putter, block *types.Block) error {
...
func WriteBlockReceipts(db lbchain-devdb.Putter, hash common.Hash, number uint64, receipts types.Receipts) error {
...
func WriteTxLookupEntries(db lbchain-devdb.Putter, block *types.Block) error {
...
func WriteBloomBits(db lbchain-devdb.Putter, bit uint, section uint64, head common.Hash, bits []byte) {
	key := append(append(bloomBitsPrefix, make([]byte, 10)...), head.Bytes()...)
...
func PreimageTable(db lbchain-devdb.Database) lbchain-devdb.Database {
	return lbchain-devdb.NewTable(db, preimagePrefix)
}

// WritePreimages writes the provided set of preimages to the database. `number` is the
// current block number, and is used for debug messages only.
func WritePreimages(db lbchain-devdb.Database, number uint64, preimages map[common.Hash][]byte) error {
...
func WriteBlockChainVersion(db lbchain-devdb.Putter, vsn int) {
	enc, _ := rlp.EncodeToBytes(uint(vsn))
	db.Put([]byte("BlockchainVersion"), enc)
}

// WriteChainConfig writes the chain config settings to the database.
func WriteChainConfig(db lbchain-devdb.Putter, hash common.Hash, cfg *params.ChainConfig) error {
