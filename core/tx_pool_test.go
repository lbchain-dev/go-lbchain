...
func setupTxPool() (*TxPool, *ecdsa.PrivateKey) {
	diskdb, _ := lbchain-devdb.NewMemDatabase()
...
func (c *testChain) State() (*state.StateDB, error) {
	...
	stdb := c.statedb
	if *c.trigger {
		db, _ := lbchain-devdb.NewMemDatabase()
		c.statedb, _ = state.New(common.Hash{}, state.NewDatabase(db))
		// simulate that the new head block included tx0 and tx1
		c.statedb.SetNonce(c.address, 2)
		c.statedb.SetBalance(c.address, new(big.Int).SetUint64(params.lbchain-dever))
		*c.trigger = false
	}
	return stdb, nil
}
...
// state reset and tests whlbchain-dever the pending state is in sync with the
// block head event that initiated the reselbchain-devate().
func Teslbchain-devateChangeDuringTransactionPoolReset(t *testing.T) {
	t.Parallel()
	...
	statedb.SetBalance(address, new(big.Int).SetUint64(params.lbchain-dever))
...
func TestInvalidTransactions(t *testing.T) {
	...
	pool.currenlbchain-devate.AddBalance(from, big.NewInt(1))
	if err := pool.AddRemote(tx); err != ErrInsufficientFunds {
		t.Error("expected", ErrInsufficientFunds)
	}

	balance := new(big.Int).Add(tx.Value(), new(big.Int).Mul(new(big.Int).SetUint64(tx.Gas()), tx.GasPrice()))
	pool.currenlbchain-devate.AddBalance(from, balance)
	if err := pool.AddRemote(tx); err != ErrIntrinsicGas {
		t.Error("expected", ErrIntrinsicGas, "got", err)
	}

	pool.currenlbchain-devate.SetNonce(from, 1)
	pool.currenlbchain-devate.AddBalance(from, big.NewInt(0xffffffffffffff))
...
func TestTransactionQueue(t *testing.T) {
	...
	tx := transaction(0, 100, key)
	from, _ := deriveSender(tx)
	pool.currenlbchain-devate.AddBalance(from, big.NewInt(1000))
	pool.lockedReset(nil, nil)
	...
	from, _ = deriveSender(tx)
	pool.currenlbchain-devate.SetNonce(from, 2)
	pool.enqueueTx(tx.Hash(), tx)
	pool.promoteExecutables([]common.Address{from})
...
func TestTransactionMissingNonce(t *testing.T) {
	...
	addr := crypto.PubkeyToAddress(key.PublicKey)
	pool.currenlbchain-devate.AddBalance(addr, big.NewInt(100000000000000))
	tx := transaction(1, 100000, key)
	if _, err := pool.add(tx, false); err != nil {
		t.Error("didn't expect error", err)
	}
...
func TestTransactionNonceRecovery(t *testing.T) {
	...
	addr := crypto.PubkeyToAddress(key.PublicKey)
	pool.currenlbchain-devate.SetNonce(addr, n)
	pool.currenlbchain-devate.AddBalance(addr, big.NewInt(100000000000000))
	pool.lockedReset(nil, nil)
	...
	pool.currenlbchain-devate.SetNonce(addr, n-1)
	pool.lockedReset(nil, nil)
...
func TestTransactionDropping(t *testing.T) {
	...
	pool.currenlbchain-devate.AddBalance(account, big.NewInt(1000))
	...
	pool.currenlbchain-devate.AddBalance(account, big.NewInt(-650))
	pool.lockedReset(nil, nil)
...
func TestTransactionPostponing(t *testing.T) {
	...
	pool.currenlbchain-devate.AddBalance(account, big.NewInt(-750))
	pool.lockedReset(nil, nil)
...
func TestTransactionGapFilling(t *testing.T) {
	...
	account, _ := deriveSender(transaction(0, 0, key))
	pool.currenlbchain-devate.AddBalance(account, big.NewInt(1000000))
...
func testTransactionQueueGlobalLimiting(t *testing.T, nolocals bool) {
	...
	for i := 0; i < len(keys); i++ {
		...
		pool.currenlbchain-devate.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000000))
	}
...
func testTransactionQueueTimeLimiting(t *testing.T, nolocals bool) {
	...
	pool.currenlbchain-devate.AddBalance(crypto.PubkeyToAddress(local.PublicKey), big.NewInt(1000000000))
	pool.currenlbchain-devate.AddBalance(crypto.PubkeyToAddress(remote.PublicKey), big.NewInt(1000000000))
...
// Tests that the transaction limits are enforced the same way irrelevant whlbchain-dever
...
func testTransactionLimitingEquivalency(t *testing.T, origin uint64) {
	...
	pool1.currenlbchain-devate.AddBalance(account1, big.NewInt(1000000))
	...
	pool2.currenlbchain-devate.AddBalance(account2, big.NewInt(1000000))
...
func TestTransactionCapClearsFromAll(t *testing.T) {
	...
	key, _ := crypto.GenerateKey()
	addr := crypto.PubkeyToAddress(key.PublicKey)
	pool.currenlbchain-devate.AddBalance(addr, big.NewInt(1000000))
...
func TestTransactionPoolRepricingKeepsLocals(t *testing.T) {
	...
	for i := 0; i < len(keys); i++ {
		keys[i], _ = crypto.GenerateKey()
		pool.currenlbchain-devate.AddBalance(crypto.PubkeyToAddress(keys[i].PublicKey), big.NewInt(1000*1000000))
	}
...
func TestTransactionReplacement(t *testing.T) {
	...
	key, _ := crypto.GenerateKey()
	pool.currenlbchain-devate.AddBalance(crypto.PubkeyToAddress(key.PublicKey), big.NewInt(1000000000))
