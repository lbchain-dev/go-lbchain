...
type ChainIndexer struct {
	chainDb  lbchain-devdb.Database      // Chain database to index the data from
	indexDb  lbchain-devdb.Database      // Prefixed table-view of the db to write index metadata into
	backend  ChainIndexerBackend // Background processor generating the index data content
	children []*ChainIndexer     // Child indexers to cascade chain updates to

	active uint32          // Flag whlbchain-dever the event loop was started
...
func NewChainIndexer(chainDb, indexDb lbchain-devdb.Database, backend ChainIndexerBackend, section, confirm uint64, throttling time.Duration, kind string) *ChainIndexer {
	c := &ChainIndexer{
		chainDb:     chainDb,
		indexDb:     indexDb,
		backend:     backend,
...
func (c *ChainIndexer) eventLoop(currentHeader *types.Header, events chan ChainEvent, sub event.Subscription) {
	...
	// Fire the initial new head event to start any oulbchain-devanding processing
