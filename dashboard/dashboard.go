...
func (db *Dashboard) collectData() {
	...
	var (
		...
		prevDiskRead       = metrics.DefaultRegistry.Get("lbchain-dev/db/chaindata/compact/input").(metrics.Meter).Count()
		prevDiskWrite      = metrics.DefaultRegistry.Get("lbchain-dev/db/chaindata/compact/output").(metrics.Meter).Count()
	...
	for {
		...
		case <-time.After(db.config.Refresh):
			...
			var (
				...
				curDiskRead       = metrics.DefaultRegistry.Get("lbchain-dev/db/chaindata/compact/input").(metrics.Meter).Count()
				curDiskWrite      = metrics.DefaultRegistry.Get("lbchain-dev/db/chaindata/compact/output").(metrics.Meter).Count()
