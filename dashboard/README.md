## Go lbchain-devchain Dashboard

The dashboard is a data visualizer integrated into glbchain-dev, intended to collect and visualize useful information of an lbchain-devchain node. It consists of two parts:
...
As the dashboard depends on certain NPM packages (which are not included in the `go-lbchain-devereum` repo), these need to be installed first:
...
Normally the dashboard assets are bundled into Glbchain-dev via `go-bindata` to avoid external dependencies. Rebuilding Glbchain-dev after each UI modification however is not feasible from a developer perspective. Instead, we can run `webpack` in watch mode to automatically rebundle the UI, and ask `glbchain-dev` to use external assets to not rely on compiled resources:

```
$ (cd dashboard/assets && ./node_modules/.bin/webpack --watch)
$ glbchain-dev --dashboard --dashboard.assets=dashboard/assets --vmodule=dashboard=5
```

To bundle up the final UI into Glbchain-dev, run `go generate`:
...
For more IDE support install the `linter-eslint` package too, which finds the `.eslintrc` file, and provides real-time linting. Atom warns, that these two packages are incompatible, but they seem to work well toglbchain-dever. For third-party library errors and auto-completion [flow-typed][flow-typed] is used.
