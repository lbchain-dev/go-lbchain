...
// Package miner implements lbchain-devchain block creation and mining.
package miner
...
type Miner struct {
	...
	canStart    int32 // can start indicates whlbchain-dever we can start the mining operation
	shouldStart int32 // should start indicates whlbchain-dever we should start after sync
}

func New(lbchain-dev Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine) *Miner {
	miner := &Miner{
		lbchain-dev:      lbchain-dev,
		mux:      mux,
		engine:   engine,
		worker:   newWorker(config, engine, common.Address{}, lbchain-dev, mux),
		canStart: 1,
	}
	miner.Register(NewCpuAgent(lbchain-dev.BlockChain(), engine))
	go miner.update()
...
func (self *Miner) Start(coinbase common.Address) {
	atomic.StoreInt32(&self.shouldStart, 1)
	self.worker.setlbchain-deverbase(coinbase)
...
func (self *Miner) Setlbchain-deverbase(addr common.Address) {
	self.coinbase = addr
	self.worker.setlbchain-deverbase(addr)
}
