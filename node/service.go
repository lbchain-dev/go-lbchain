...
func (ctx *ServiceContext) OpenDatabase(name string, cache int, handles int) (lbchain-devdb.Database, error) {
	if ctx.config.DataDir == "" {
		return lbchain-devdb.NewMemDatabase()
	}
	db, err := lbchain-devdb.NewLDBDatabase(ctx.config.resolvePath(name), cache, handles)
	if err != nil {
		return nil, err
	}
	return db, nil
