...
type Config struct {
	...
	// used in the devp2p node identifier. The instance name of glbchain-dev is "glbchain-dev". If no
...
func (c *Config) NodeName() string {
	name := c.name()
	// Backwards compatibility: previous versions used title-cased "Glbchain-dev", keep that.
	if name == "glbchain-dev" || name == "glbchain-dev-testnet" {
		name = "Glbchain-dev"
	}
	if c.UserIdent != "" {
		name += "/" + c.UserIdent
...
// These resources are resolved differently for "glbchain-dev" instances.
var isOldGlbchain-devResource = map[string]bool{
	"chaindata":          true,
	"nodes":              true,
	"nodekey":            true,
...
func (c *Config) resolvePath(path string) string {
	...
	// by glbchain-dev 1.4 are used if they exist.
	if c.name() == "glbchain-dev" && isOldGlbchain-devResource[path] {
		oldpath := ""
		if c.Name == "glbchain-dev" {
...
func makeAccountManager(conf *Config) (*accounts.Manager, string, error) {
	...
	if keydir == "" {
		// There is no datadir.
		keydir, err = ioutil.TempDir("", "go-lbchain-devereum-keystore")
		ephemeral = keydir
	}
