...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/p2p/simulations/adapters"
)
...
// starting nodes and connecting them toglbchain-dever.
//
...
// Connect connects two nodes toglbchain-dever by calling the "admin_addPeer" RPC
...
// NOTE:
// it also checks whlbchain-dever there has been recent attempt to connect the peers
...
type Node struct {
	...
	// Up tracks whlbchain-dever or not the node is running
	Up bool `json:"up"`
}
...
type Conn struct {
	...
	// Up tracks whlbchain-dever or not the connection is active
	Up bool `json:"up"`
...
// nodesUp returns whlbchain-dever both nodes are currently up
