...
creating a collection of devp2p nodes, connecting them toglbchain-dever to form a
...
connecting two nodes toglbchain-dever).

This is in contrast to a non-control event, otherwise called a "live" event,
which is the outcome of somlbchain-deving happening in the network as a result of a
...
* `Expect` - an expectation function which returns whlbchain-dever or not a
...
As a concrete example, consider a simulated network of lbchain-devchain nodes. An
