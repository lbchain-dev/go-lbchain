...
func (t *TestAPI) Gelbchain-devate() []byte {
	return t.state.Load().([]byte)
}

func (t *TestAPI) Selbchain-devate(state []byte) {
	t.state.Store(state)
}
...
func TestHTTPSnapshot(t *testing.T) {
	...
	for i, node := range nodes {
		...
		if err := rpc.Call(nil, "test_selbchain-devate", []byte(state)); err != nil {
	...
	for i, state := range states {
		golbchain-devate := snap.Nodes[i].Snapshots["test"]
		if string(golbchain-devate) != state {
			t.Fatalf("expected snapshot state %q, got %q", state, golbchain-devate)
		}
	}
	...
	for i, node := range nodes {
		...
		defer rpc.Close()
		var state []byte
		if err := rpc.Call(&state, "test_gelbchain-devate"); err != nil {
