...
type tickelbchain-devore struct {
...
func newTickelbchain-devore() *tickelbchain-devore {
	return &tickelbchain-devore{
...
func (s *tickelbchain-devore) addTopic(topic Topic, register bool) {
...
func (s *tickelbchain-devore) addSearchTopic(t Topic, foundChn chan<- *Node) {
	s.addTopic(t, false)
...
func (s *tickelbchain-devore) removeSearchTopic(t Topic) {
...
func (s *tickelbchain-devore) removeRegisterTopic(topic Topic) {
...
func (s *tickelbchain-devore) regTopicSet() []Topic {
...
func (s *tickelbchain-devore) nextRegisterLookup() (lookupInfo, time.Duration) {
...
func (s *tickelbchain-devore) nextSearchLookup(topic Topic) lookupInfo {
	tr := s.radius[topic]
...
func (s *tickelbchain-devore) ticketsInWindow(topic Topic) []ticketRef {
...
func (s *tickelbchain-devore) removeExcessTickets(t Topic) {
...
// Less reports whlbchain-dever the element with
...
func (s *tickelbchain-devore) addTicketRef(r ticketRef) {
...
func (s *tickelbchain-devore) nextFilteredTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
	for {
...
func (s *tickelbchain-devore) ticketRegistered(ref ticketRef) {
	now := mclock.Now()
...
func (s *tickelbchain-devore) nextRegisterableTicket() (*ticketRef, time.Duration) {
	now := mclock.Now()
...
func (s *tickelbchain-devore) removeTicketRef(ref ticketRef) {
...
func (s *tickelbchain-devore) registerLookupDone(lookup lookupInfo, nodes []*Node, ping func(n *Node) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
...
func (s *tickelbchain-devore) searchLookupDone(lookup lookupInfo, nodes []*Node, query func(n *Node, topic Topic) []byte) {
	now := mclock.Now()
	for i, n := range nodes {
		if i == 0 || (binary.BigEndian.Uint64(n.sha[:8])^binary.BigEndian.Uint64(lookup.target[:8])) < s.radius[lookup.topic].minRadius {
			if lookup.radiusLookup {
...
func (s *tickelbchain-devore) adjustWithTicket(now mclock.AbsTime, targetHash common.Hash, t *ticket) {
...
func (s *tickelbchain-devore) addTicket(localTime mclock.AbsTime, pingHash []byte, ticket *ticket) {
...
func (s *tickelbchain-devore) getNodeTicket(node *Node) *ticket {
	if s.nodes[node] == nil {
...
func (s *tickelbchain-devore) canQueryTopic(node *Node, topic Topic) bool {
...
func (s *tickelbchain-devore) addTopicQuery(hash common.Hash, node *Node, lookup lookupInfo) {
	now := mclock.Now()
	qq := s.queriesSent[node]
	if qq == nil {
...
func (s *tickelbchain-devore) cleanupTopicQueries(now mclock.AbsTime) {
...
func (s *tickelbchain-devore) gotTopicNodes(from *Node, hash common.Hash, nodes []rpcNode) (timeout bool) {
	now := mclock.Now()
