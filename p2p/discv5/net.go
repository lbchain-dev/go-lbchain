...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/crypto/sha3"
...
type Network struct {
	...
	tab           *Table
	topictab      *topicTable
	tickelbchain-devore   *tickelbchain-devore
	nursery       []*Node
...
func newNetwork(conn transport, ourPubkey ecdsa.PublicKey, dbPath string, netrestrict *netutil.Netlist) (*Network, error) {
	...
	net := &Network{
		...
		tickelbchain-devore:      newTickelbchain-devore(),
...
func (net *Network) SetFallbackNodes(nodes []*Node) error {
	...
	for _, n := range nodes {
		if err := n.validateComplete(); err != nil {
			return fmt.Errorf("bad boolbchain-devrap/fallback node %q (%v)", n, err)
		}
...
func (net *Network) loop() {
	...
	resetNextTicket := func() {
		ticket, timeout := net.tickelbchain-devore.nextFilteredTicket()
...
loop:
	for {
		...
		case req := <-net.topicRegisterReq:
			...
			if !req.add {
				net.tickelbchain-devore.removeRegisterTopic(req.topic)
				continue
			}
			net.tickelbchain-devore.addTopic(req.topic, true)
			...
			if topicRegisterLookupTarget.target == (common.Hash{}) {
				...
				target, delay := net.tickelbchain-devore.nextRegisterLookup()
		...
		case nodes := <-topicRegisterLookupDone:
			...
			net.tickelbchain-devore.registerLookupDone(topicRegisterLookupTarget, nodes, func(n *Node) []byte {
				net.ping(n, n.addr())
				return n.pingEcho
			})
			target, delay := net.tickelbchain-devore.nextRegisterLookup()
		...
		case <-nextRegisterTime:
			...
			net.tickelbchain-devore.ticketRegistered(*nextTicket)
			...
			if refreshDone == nil {
				...
				if ok {
					if req.delay == time.Duration(0) {
						...
						net.tickelbchain-devore.removeSearchTopic(req.topic)
					} else {
				...
				if req.delay != time.Duration(0) {
					...
					searchInfo[req.topic] = info
					net.tickelbchain-devore.addSearchTopic(req.topic, req.found)
		...
		case topic := <-topicSearch:
			if activeSearchCount < maxSearchCount {
				activeSearchCount++
				target := net.tickelbchain-devore.nextSearchLookup(topic)
				go func() {
		...
		case res := <-topicSearchLookupDone:
			...
			if lookupChn := searchInfo[res.target.topic].lookupChn; lookupChn != nil {
				lookupChn <- net.tickelbchain-devore.radius[res.target.topic].converged
			}
			net.tickelbchain-devore.searchLookupDone(res.target, res.nodes, func(n *Node, topic Topic) []byte {
		...
		case <-statsDump.C:
			...
			/*r, ok := net.tickelbchain-devore.radius[testTopic]
			if !ok {
				fmt.Printf("(%x) no radius @ %v\n", net.tab.self.ID[:8], time.Now())
			} else {
				topics := len(net.tickelbchain-devore.tickets)
				tickets := len(net.tickelbchain-devore.nodes)
				rad := r.radius / (maxRadius/10000+1)
				fmt.Printf("(%x) topics:%d radius:%d tickets:%d @ %v\n", net.tab.self.ID[:8], topics, rad, tickets, time.Now())
			}*/

			tm := mclock.Now()
			for topic, r := range net.tickelbchain-devore.radius {
				if printTestImgLogs {
...
func (net *Network) refresh(done chan<- struct{}) {
	...
	for _, n := range seeds {
		...
		// Force-add the seed node so Lookup does somlbchain-deving.
...
func (net *Network) ping(n *Node, addr *net.UDPAddr) {
	...
	n.pingTopics = net.tickelbchain-devore.regTopicSet()
...
func (net *Network) handleKnownPong(n *Node, pkt *ingressPacket) error {
	...
	if err == nil {
		...
		net.tickelbchain-devore.addTicket(now, pkt.data.(*pong).ReplyTok, ticket)
	} else {
...
func (net *Network) handleQueryEvent(n *Node, ev nodeEvent, pkt *ingressPacket) (*nodeState, error) {
	...
	case topicQueryPacket:
		...
		if _, ok := net.tickelbchain-devore.tickets[topic]; ok {
	...
	case topicNodesPacket:
		...
		if net.tickelbchain-devore.gotTopicNodes(n, p.Echo, p.Nodes) {
			n.queryTimeouts++
			if n.queryTimeouts > maxFindnodeFailures && n.state == known {
				return contested, errors.New("too many timeouts")
			}
