...
type tester struct {
	workspace string
	stack     *node.Node
	lbchain-devereum  *lbchain-dev.lbchain-devchain
	console   *Console
...
func newTester(t *testing.T, confOverride func(*lbchain-dev.Config)) *tester {
	...
	// Create a networkless protocol stack and start an lbchain-devchain service within
	stack, err := node.New(&node.Config{DataDir: workspace, UseLightweightKDF: true, Name: testInstance})
	if err != nil {
		t.Fatalf("failed to create node: %v", err)
	}
	lbchain-devConf := &lbchain-dev.Config{
		Genesis:   core.DeveloperGenesisBlock(15, common.Address{}),
		lbchain-deverbase: common.HexToAddress(testAddress),
		lbchain-devash: ethash.Config{
			PowMode: ethash.ModeTest,
		},
	}
	if confOverride != nil {
		confOverride(lbchain-devConf)
	}
	if err = stack.Register(func(ctx *node.ServiceContext) (node.Service, error) { return lbchain-dev.New(ctx, lbchain-devConf) }); err != nil {
		t.Fatalf("failed to register lbchain-devchain protocol: %v", err)
	}
	...
	stack.Service(&lbchain-devereum)

	return &tester{
		workspace: workspace,
		stack:     stack,
		lbchain-devereum:  lbchain-devereum,
		console:   console,
		input:     prompter,
		output:    printer,
	}
