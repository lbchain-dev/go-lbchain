...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/internal/jsre"
	"github.com/lbchain-devchain/go-lbchain-dev/internal/web3ext"
...
type Config struct {
	...
	Client   *rpc.Client  // RPC client to execute lbchain-devchain requests through
...
func (c *Console) init(preload []string) error {
	...
	c.jsre.Set("jlbchain-dev", struct{}{})

	jlbchain-devObj, _ := c.jsre.Get("jlbchain-dev")
	jlbchain-devObj.Object().Set("send", bridge.Send)
	jlbchain-devObj.Object().Set("sendAsync", bridge.Send)
	...
	if _, err := c.jsre.Run("var web3 = new Web3(jlbchain-dev);"); err != nil {
	...
	flatten := "var lbchain-dev = web3.lbchain-dev; var personal = web3.personal; "
	for api := range apis {
		if api == "web3" {
	...
	//c.jsre.Run(`var GlobalRegistrar = lbchain-dev.contract(` + registrar.GlobalRegistrarAbi + `);   registrar = GlobalRegistrar.at("` + registrar.GlobalRegistrarAddr + `");`)
	...
	if c.prompter != nil {
		...
		// original web3 callbacks. These will be called by the jlbchain-dev.* methods after
		// they got the password from the user and send the original web3 request to
		// the backend.
		if obj := personal.Object(); obj != nil { // make sure the personal api is enabled over the interface
			if _, err = c.jsre.Run(`jlbchain-dev.openWallet = personal.openWallet;`); err != nil {
				return fmt.Errorf("personal.openWallet: %v", err)
			}
			if _, err = c.jsre.Run(`jlbchain-dev.unlockAccount = personal.unlockAccount;`); err != nil {
				return fmt.Errorf("personal.unlockAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jlbchain-dev.newAccount = personal.newAccount;`); err != nil {
				return fmt.Errorf("personal.newAccount: %v", err)
			}
			if _, err = c.jsre.Run(`jlbchain-dev.sign = personal.sign;`); err != nil {
...
func (c *Console) AutoCompleteInput(line string, pos int) (string, []string, string) {
	...
	// E.g. in case of nested lines lbchain-dev.getBalance(lbchain-dev.coinb<tab><tab>
	start := pos - 1
	for ; start > 0; start-- {
...
// Welcome show summary of current Glbchain-dev instance and some metadata about the
// console's available modules.
func (c *Console) Welcome() {
	// Print some generic Glbchain-dev metadata
	fmt.Fprintf(c.printer, "Welcome to the Glbchain-dev JavaScript console!\n\n")
	c.jsre.Run(`
		console.log("instance: " + web3.version.node);
		console.log("coinbase: " + lbchain-dev.coinbase);
		console.log("at block: " + lbchain-dev.blockNumber + " (" + new Date(1000 * lbchain-dev.getBlock(lbchain-dev.blockNumber).timestamp) + ")");
