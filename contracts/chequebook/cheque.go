...
// Package chequebook package wraps the 'chequebook' lbchain-devchain smart contract.
//
// The functions in this package allow using chequebook for
// issuing, receiving, verifying cheques in lbchain-dever; (auto)cashing cheques in lbchain-dever
// as well as (auto)depositing lbchain-dever to the chequebook contract.
package chequebook
...
import (
	...
	"github.com/lbchain-devchain/go-lbchain-dev/contracts/chequebook/contract"
	...
	"github.com/lbchain-devchain/go-lbchain-dev/swarm/services/swap/swap"
)
...
// * depositing lbchain-dever to the chequebook
// * watching incoming lbchain-dever
...
func (self *Chequebook) UnmarshalJSON(data []byte) error {
	...
	_, ok := self.balance.Selbchain-devring(file.Balance, 10)
	if !ok {
	...
	for addr, sent := range file.Sent {
		self.sent[common.HexToAddress(addr)], ok = new(big.Int).Selbchain-devring(sent, 10)
		if !ok {
...
// Cash cashes the cheque by sending an lbchain-devchain transaction.
